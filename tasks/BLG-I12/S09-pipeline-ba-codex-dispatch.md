...
Reasons:
- stage S09 / role pipeline-ba default tier is moderate
...
```json
{
  ...
  "backlog_item_id": "BLG-I12",
  "backlog_item": {
    "id": "BLG-I12",
    "title": "Предупреждать о закрытии с несохранёнными изменениями",
    "source_section": "I12, клиентский список 6",
  ...
  "budget_usage": {
    "input_tokens": 15300,
    "output_tokens": 2300,
    "estimated_usd": 0.2525
  },
  "blocked_by": [],
  "blocker": {
    "type": "ENV",
    "reason_code": "PIPELINE_REWORK_ROUTE_BROKEN",
    "details": "Controller recorded PRODUCT_REJECTED with resume_stage S09, but packet ignored REWORK current_stage and routed BLG-I12 to S01 after clearing accepted S01/S02 receipts.",
    "owner": "pipeline-controller-maintainer",
    "created_at": "2026-08-21T00:56:57Z",
    "resume_stage": "S09"
  },
  "resume_condition": {
    "stage": "S09",
    "condition": "Repair or authorize the Pipeline controller rework route so BLG-I12 resumes at S09 without replaying accepted S01/S02, then regenerate packet and dispatch."
  },
  "rules": [
...
```bash
python3 scripts/pipeline/run.py next --task-id BLG-I12
python3 scripts/pipeline/run.py validate --task-id BLG-I12
```
...
```bash
python3 scripts/pipeline/run.py advance --task-id BLG-I12 --stage S09 --verdict <ALLOWED_VERDICT> --role pipeline-ba --agent <agent-id> --executor codex --model gpt-5.6-terra --tier moderate --input-tokens <INPUT_TOKENS> --output-tokens <OUTPUT_TOKENS> --estimated-usd <USD>
python3 scripts/pipeline/run.py packet --task-id BLG-I12
python3 scripts/pipeline/dispatch.py --task-id BLG-I12 --executor codex
```
