...
```json
{
  ...
  "blocker": {
    "type": "OWNER_INPUT",
    "reason_code": "BLK_PROD_001_RESEARCH_BLOCKED",
    "details": "S04 cannot issue RESEARCH_PASSED: the S03 receipt has empty output_hashes and no versioned external-contract research artifact or source-backed decision table exists; BLK-PROD-001 remains open.",
    "owner": "pipeline-product",
    "created_at": "2026-08-21T00:59:34Z",
    "resume_stage": "S04"
  },
  "resume_condition": {
    "stage": "S04",
    "condition": "Provide the minimum closure artifact: approved oracle/decision record for requiredMeta, optionalMeta and sgtinApplied, plus a versioned S03 external-contract artifact with official source/version/date and field/status semantics for independent S04 review."
  },
  "rules": [
...
```bash
python3 scripts/pipeline/run.py advance --task-id BLG-D03 --stage S04 --verdict <ALLOWED_VERDICT> --role pipeline-reviewer --agent <agent-id> --executor codex --model gpt-5.6-sol --tier expensive --input-tokens <INPUT_TOKENS> --output-tokens <OUTPUT_TOKENS> --estimated-usd <USD>
