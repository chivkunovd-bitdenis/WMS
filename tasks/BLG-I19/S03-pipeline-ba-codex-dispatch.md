...
```json
{
  ...
  "backlog_item_id": "BLG-I19",
  "backlog_item": {
    "id": "BLG-I19",
    "title": "Повторять массовые операции с WB после rate limit",
    "source_section": "I19",
  ...
  "budget_usage": {
    "input_tokens": 30300,
    "output_tokens": 8600,
    "estimated_usd": 0.7225
  },
  "blocked_by": [],
  "blocker": {
    "type": "BASELINE",
    "reason_code": "RESEARCH_REWORK_WB_DELETE_RATE_LIMIT_CONTRACT",
    "details": "S04 RESEARCH_REWORK: current FBS OpenAPI counts every 4XX as 10 requests; DELETE key=sgtin removes the whole 1..100-code value while S03 tracks only order-level progress; and the Marketplace sandbox limit of 1 request/second is missing.",
    "owner": "pipeline-ba",
    "created_at": "2026-08-21T00:53:31Z",
    "resume_stage": "S03"
  },
  "resume_condition": {
    "stage": "S03",
    "condition": "Revise both S03 artifacts for all-4XX x10 accounting, explicit delete-all versus subset semantics with per-code outcomes and recovery, and sandbox pacing; then rerun an independent S04."
  },
  "rules": [
...
```bash
python3 scripts/pipeline/run.py next --task-id BLG-I19
python3 scripts/pipeline/run.py validate --task-id BLG-I19
```
...
```bash
python3 scripts/pipeline/run.py advance --task-id BLG-I19 --stage S03 --verdict <ALLOWED_VERDICT> --role pipeline-ba --agent <agent-id> --executor codex --model gpt-5.6-sol --tier expensive --input-tokens <INPUT_TOKENS> --output-tokens <OUTPUT_TOKENS> --estimated-usd <USD>
python3 scripts/pipeline/run.py packet --task-id BLG-I19
python3 scripts/pipeline/dispatch.py --task-id BLG-I19 --executor codex
```
