...
```json
{
  ...
  "backlog_item_id": "BLG-D08",
  "backlog_item": {
    "id": "BLG-D08",
    "title": "Удалить или реально подключить мёртвый экран FfFbsPickList",
    "source_section": "D8",
...
```bash
python3 scripts/pipeline/run.py next --task-id BLG-D08
python3 scripts/pipeline/run.py validate --task-id BLG-D08
```
...
```bash
python3 scripts/pipeline/run.py advance --task-id BLG-D08 --stage S06 --verdict <ALLOWED_VERDICT> --role pipeline-ba --agent <agent-id> --executor codex --model gpt-5.6-terra --tier moderate --input-tokens <INPUT_TOKENS> --output-tokens <OUTPUT_TOKENS> --estimated-usd <USD>
python3 scripts/pipeline/run.py packet --task-id BLG-D08
python3 scripts/pipeline/dispatch.py --task-id BLG-D08 --executor codex
```
