...
Безопасный статус нельзя свести к одному флагу «ключ есть». Для каждого селлера и каждой реально используемой WMS capability (`content`, `supplies`, `marketplace`) нужно отдельно хранить: настроен ли секрет, результат последней проверки нужного домена, время проверки, время последнего успеха, срок по локально разобранной JWT metadata и последнюю безопасную категорию ошибки.
...
Значение токена, `Authorization`, части токена, JWT identifiers (`id`, `sid`, `asid`), битовая маска категорий и произвольное upstream body не должны возвращаться в UI, логи аналитики или receipt. Оператор видит только allowlisted статус и действие для ответственного сотрудника. Управление ключом остаётся вне операторского сценария.
...
### 2. Probe по capability

| Capability WMS | Неразрушающий probe | Что доказывает `200` |
|---|---|---|
| `content` | `GET https://content-api.wildberries.ru/ping` | токен/URL/категория подходят сервису Content |
| `supplies` | `GET https://supplies-api.wildberries.ru/ping` | токен/URL/категория подходят сервису Supplies |
| `marketplace` | `GET https://marketplace-api.wildberries.ru/ping` | токен/URL/категория подходят сервису Marketplace |
...
| transport/DNS/timeout | `NETWORK_UNAVAILABLE` | «Не удалось связаться с WB; сохранён последний известный статус.» | что ключ просрочен |
| HTTP `5xx` | `WB_UNAVAILABLE` | «WB временно не отвечает; сохранён последний известный статус.» | что ключ неисправен |
...
Запрещённые поля: `token`, `token_mask`, `authorization`, `jwt`, `sid`, `asid`, `token_id`, `raw_detail`, `raw_response`, `request_headers`.
...
4. **Required later, S15:** без реальных ключей зафиксировать emulator cases: missing, malformed, expired, revoked/unknown `401`, wrong category `403`, read-only `401/403`, `429` с retry, timeout, `5xx`, recovery и отсутствие secret leakage.

## Решение S03

`RESEARCH_READY`: узкий внешний контракт собран, все применимые lanes обработаны, live credentials не использовались. Независимая критика обязательна на S04.
