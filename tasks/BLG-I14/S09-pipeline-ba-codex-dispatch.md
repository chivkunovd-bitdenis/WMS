...
```json
{
  ...
  "backlog_item_id": "BLG-I14",
  "backlog_item": {
    "id": "BLG-I14",
    "title": "Запрещать ПВЗ, если часть заказов нельзя сдавать через ПВЗ",
    "source_section": "I14",
    "business_meaning": "Wildberries передаёт для каждого заказа признак, разрешена ли его сдача через пункт выдачи, и WMS уже сохраняет это значение в can_pvz. При выборе способа сдачи система не проверяет весь состав поставки, поэтому оператор может выбрать ПВЗ для смешанной пачки, где часть заказов туда везти нельзя. Перед подтверждением нужно проверить все заказы, заблокировать недопустимый способ, показать точное число мешающих заказов и дать возможность увидеть их список.",
    "type": "ui_change",
    "priority": "high",
    "status": "queued",
...
```bash
python3 scripts/pipeline/run.py next --task-id BLG-I14
python3 scripts/pipeline/run.py validate --task-id BLG-I14
```
...
```bash
python3 scripts/pipeline/run.py advance --task-id BLG-I14 --stage S09 --verdict <ALLOWED_VERDICT> --role pipeline-ba --agent <agent-id> --executor codex --model gpt-5.6-terra --tier moderate --input-tokens <INPUT_TOKENS> --output-tokens <OUTPUT_TOKENS> --estimated-usd <USD>
python3 scripts/pipeline/run.py packet --task-id BLG-I14
python3 scripts/pipeline/dispatch.py --task-id BLG-I14 --executor codex
```
