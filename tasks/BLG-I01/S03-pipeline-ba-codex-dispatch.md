...
```json
{
  ...
  "backlog_item_id": "BLG-I01",
  "backlog_item": {
    "id": "BLG-I01",
    "title": "Перейти на актуальный WB endpoint статусов маркировки",
    "source_section": "I1",
  ...
  "blocker": {
    ...
    "created_at": "2026-08-21T00:28:11Z",
    "resume_stage": "S03"
  },
  "resume_condition": {
    "stage": "S03",
...
```bash
python3 scripts/pipeline/run.py next --task-id BLG-I01
python3 scripts/pipeline/run.py validate --task-id BLG-I01
```
...
```bash
python3 scripts/pipeline/run.py advance --task-id BLG-I01 --stage S03 --verdict <ALLOWED_VERDICT> --role pipeline-ba --agent <agent-id> --executor codex --model gpt-5.6-sol --tier expensive --input-tokens <INPUT_TOKENS> --output-tokens <OUTPUT_TOKENS> --estimated-usd <USD>
python3 scripts/pipeline/run.py packet --task-id BLG-I01
python3 scripts/pipeline/dispatch.py --task-id BLG-I01 --executor codex
```
