...
Reasons:
- stage S12 / role pipeline-ba default tier is moderate
...
```json
{
  ...
  "backlog_item_id": "BLG-D14",
  "backlog_item": {
    "id": "BLG-D14",
    "title": "Устранить deadlock при создании крупной поставки и автоопросе",
    "source_section": "D14",
  ...
  "budget_usage": {
    "input_tokens": 65300,
    "output_tokens": 3600,
    "estimated_usd": 0.8525
  },
  "blocked_by": [
    {
      "id": "BLK-ARCH-002",
      "title": "Для крупных операций не закреплён безопасный concurrency-контракт",
      "status": "open",
      "type": "architecture",
      "owner_role": "solution-architect",
      "resume_stage": "S13",
      "minimum_closure_artifact": "lock ordering, retry/timeout policy and load case for at least 155 orders"
    }
  ],
  "blocker": null,
...
```bash
python3 scripts/pipeline/run.py next --task-id BLG-D14
python3 scripts/pipeline/run.py validate --task-id BLG-D14
```
...
```bash
python3 scripts/pipeline/run.py advance --task-id BLG-D14 --stage S12 --verdict <ALLOWED_VERDICT> --role pipeline-ba --agent <agent-id> --executor codex --model gpt-5.6-terra --tier moderate --input-tokens <INPUT_TOKENS> --output-tokens <OUTPUT_TOKENS> --estimated-usd <USD>
python3 scripts/pipeline/run.py packet --task-id BLG-D14
python3 scripts/pipeline/dispatch.py --task-id BLG-D14 --executor codex
```
