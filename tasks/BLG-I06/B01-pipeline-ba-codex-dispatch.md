...
```json
{
  ...
  "backlog_item_id": "BLG-I06",
  "backlog_item": {
    "id": "BLG-I06",
    "title": "Обновлять idempotency key при смене способа сдачи",
    "source_section": "I6",
  ...
  "blocker": {
    ...
    "details": "B01 requires isolated baseline reproduction fixture for: Обновлять idempotency key при смене способа сдачи. Backlog business_meaning describes the symptom, but no runnable fixture, test case, screen state, or observation log was supplied in the task packet.",
    "owner": "pipeline-ba",
    "created_at": "2026-08-21T00:27:47Z",
    "resume_stage": "B01"
  },
  "resume_condition": {
    "stage": "B01",
...
```bash
python3 scripts/pipeline/run.py next --task-id BLG-I06
python3 scripts/pipeline/run.py validate --task-id BLG-I06
```
...
```bash
python3 scripts/pipeline/run.py advance --task-id BLG-I06 --stage B01 --verdict <ALLOWED_VERDICT> --role pipeline-ba --agent <agent-id> --executor codex --model gpt-5.6-terra --tier moderate --input-tokens <INPUT_TOKENS> --output-tokens <OUTPUT_TOKENS> --estimated-usd <USD>
python3 scripts/pipeline/run.py packet --task-id BLG-I06
python3 scripts/pipeline/dispatch.py --task-id BLG-I06 --executor codex
```
