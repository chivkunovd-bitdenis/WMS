...
Reasons:
- stage S20 / role pipeline-reviewer default tier is expensive
...
```bash
python3 scripts/pipeline/run.py advance --task-id BLG-J02 --stage S20 --verdict <ALLOWED_VERDICT> --role pipeline-reviewer --agent <agent-id> --executor codex --model gpt-5.6-sol --tier expensive --input-tokens <INPUT_TOKENS> --output-tokens <OUTPUT_TOKENS> --estimated-usd <USD>
