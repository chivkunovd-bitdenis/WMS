...
```json
{
  ...
  "backlog_item_id": "BLG-D02",
  "backlog_item": {
    "id": "BLG-D02",
    "title": "Создавать задание упаковки при зеркалировании поставки WB",
    "source_section": "D2",
  ...
  "blocker": {
    ...
    "details": "B01 requires isolated baseline reproduction fixture for: Создавать задание упаковки при зеркалировании поставки WB. Backlog business_meaning describes the symptom, but no runnable fixture, test case, screen state, or observation log was supplied in the task packet.",
    "owner": "pipeline-ba",
...
```bash
python3 scripts/pipeline/run.py next --task-id BLG-D02
python3 scripts/pipeline/run.py validate --task-id BLG-D02
```
...
```bash
python3 scripts/pipeline/run.py advance --task-id BLG-D02 --stage B01 --verdict <ALLOWED_VERDICT> --role pipeline-ba --agent <agent-id> --executor codex --model gpt-5.6-terra --tier moderate --input-tokens <INPUT_TOKENS> --output-tokens <OUTPUT_TOKENS> --estimated-usd <USD>
python3 scripts/pipeline/run.py packet --task-id BLG-D02
python3 scripts/pipeline/dispatch.py --task-id BLG-D02 --executor codex
```
