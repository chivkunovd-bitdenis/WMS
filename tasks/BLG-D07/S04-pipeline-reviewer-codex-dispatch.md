...
```json
{
  ...
  "backlog_item_id": "BLG-D07",
  "backlog_item": {
    "id": "BLG-D07",
    "title": "Добавить повторную сверку статусов заказов WB",
    "source_section": "D7",
  ...
  "budget_usage": {
    "input_tokens": 82300,
    "output_tokens": 14100,
    "estimated_usd": 1.1525
  },
  "blocked_by": [],
  "blocker": {
    "type": "ORACLE_CONFLICT",
    "reason_code": "RESEARCH_REWORK_WB_STATUS_CONTRACT",
    "details": "S03 has an unresolved WB status/error contract: postponed_delivery is omitted, carrier cancel values lack reproducible provenance, and 4XX x10 is incorrect because the documented multiplier applies to 409; sandbox cases are documented designs, not executed proof.",
    "owner": "pipeline-ba",
    "created_at": "2026-08-21T00:46:33Z",
    "resume_stage": "S04"
  },
  "resume_condition": {
    "stage": "S04",
    "condition": "Correct S03 status and error matrices with dated reproducible official provenance, state 409 x10 precisely, distinguish documented sandbox scenarios from executed proof, and hand explicit local-emulator coverage to S15; then rerun independent S04."
  },
  "rules": [
...
```bash
python3 scripts/pipeline/run.py next --task-id BLG-D07
python3 scripts/pipeline/run.py validate --task-id BLG-D07
```
...
```bash
python3 scripts/pipeline/run.py advance --task-id BLG-D07 --stage S04 --verdict <ALLOWED_VERDICT> --role pipeline-reviewer --agent <agent-id> --executor codex --model gpt-5.6-sol --tier expensive --input-tokens <INPUT_TOKENS> --output-tokens <OUTPUT_TOKENS> --estimated-usd <USD>
python3 scripts/pipeline/run.py packet --task-id BLG-D07
python3 scripts/pipeline/dispatch.py --task-id BLG-D07 --executor codex
```
