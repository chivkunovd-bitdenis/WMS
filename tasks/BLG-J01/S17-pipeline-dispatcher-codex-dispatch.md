...
Reasons:
- stage S17 / role pipeline-dispatcher default tier is moderate
...
```json
{
  ...
  "backlog_item_id": "BLG-J01",
  "backlog_item": {
    "id": "BLG-J01",
    "title": "Подписать отображаемый хвост кода маркировки",
    "source_section": "J, хвост кода",
...
```bash
python3 scripts/pipeline/run.py next --task-id BLG-J01
python3 scripts/pipeline/run.py validate --task-id BLG-J01
```
...
```bash
python3 scripts/pipeline/run.py advance --task-id BLG-J01 --stage S17 --verdict <ALLOWED_VERDICT> --role pipeline-dispatcher --agent <agent-id> --executor codex --model gpt-5.6-terra --tier moderate --input-tokens <INPUT_TOKENS> --output-tokens <OUTPUT_TOKENS> --estimated-usd <USD>
python3 scripts/pipeline/run.py packet --task-id BLG-J01
python3 scripts/pipeline/dispatch.py --task-id BLG-J01 --executor codex
```
