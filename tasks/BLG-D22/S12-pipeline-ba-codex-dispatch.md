...
```json
{
  ...
  "backlog_item_id": "BLG-D22",
  "backlog_item": {
    "id": "BLG-D22",
    "title": "Починить полный pytest-гейт и разделить причины зависания",
    "source_section": "D22",
  ...
  "budget_usage": {
    "input_tokens": 42300,
    "output_tokens": 4300,
    "estimated_usd": 0.3225
  },
  "blocked_by": [
    {
      "id": "BLK-TEST-001",
      "title": "Полный pytest-прогон не является работающим гейтом",
      "status": "open",
      "type": "test",
      "owner_role": "pipeline-reviewer",
      "resume_stage": "S15",
...
```bash
python3 scripts/pipeline/run.py next --task-id BLG-D22
python3 scripts/pipeline/run.py validate --task-id BLG-D22
```
...
```bash
python3 scripts/pipeline/run.py advance --task-id BLG-D22 --stage S12 --verdict <ALLOWED_VERDICT> --role pipeline-ba --agent <agent-id> --executor codex --model gpt-5.6-terra --tier moderate --input-tokens <INPUT_TOKENS> --output-tokens <OUTPUT_TOKENS> --estimated-usd <USD>
python3 scripts/pipeline/run.py packet --task-id BLG-D22
python3 scripts/pipeline/dispatch.py --task-id BLG-D22 --executor codex
```
