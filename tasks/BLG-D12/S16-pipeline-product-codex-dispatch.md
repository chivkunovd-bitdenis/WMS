...
```bash
python3 scripts/pipeline/run.py advance --task-id BLG-D12 --stage S16 --verdict <ALLOWED_VERDICT> --role pipeline-product --agent <agent-id> --executor codex --model gpt-5.6-sol --tier expensive --input-tokens <INPUT_TOKENS> --output-tokens <OUTPUT_TOKENS> --estimated-usd <USD>
