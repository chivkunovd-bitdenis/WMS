...
```bash
python3 scripts/pipeline/run.py advance --task-id BLG-F1A --stage S13 --verdict <ALLOWED_VERDICT> --role solution-architect --agent <agent-id> --executor codex --model gpt-5.6-sol --tier expensive --input-tokens <INPUT_TOKENS> --output-tokens <OUTPUT_TOKENS> --estimated-usd <USD>
