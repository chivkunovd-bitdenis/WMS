...
## TASK-136 — 2026-08-02 — STOCKFIX-020/030/055 integrated

- What changed: **020** Celery stock sync 202 JobOut (union `response_model` + mocked `.delay` test). **030** deliver only after packaging (`packaging_required`). **055** binding `last_error_code` = real PUT/transport error (not false `readback_mismatch`).
- What did NOT change: operator UI (080); sold-unit regression (035); full ruff/mypy/pytest (090).
- Verification: verifiers READY; adversarial APPROVE WITH WARNINGS.
- Integration HEAD: `9d20f1e` on `feat/fbs-stock-sync`.

## TASK-135 — 2026-08-02 — STOCKFIX-050 atomic FBS stock sync lease

- What changed: atomic `_try_acquire_lease`; concurrency test. Commit `ef4c4bd` / integrate `f15cea8`.

## TASK-134 — 2026-08-02 — STOCKFIX-070 FBS warehouse remap conflict

- What changed: fail-closed remap → `warehouse_remap_conflict`. Commit `b9c66d5` / integrate `f7019e6`.

## TASK-133 — 2026-08-02 — STOCKFIX-060 binding disable blocked by reserves

- What changed: `disable_binding()` → 409 `active_fbs_reservations`. Commit `23692c4` / integrate `7371e1c`.

## TASK-132 — 2026-08-02 — STOCKFIX-010 + STOCKFIX-040 integrated

- What changed: tracked Alembic 0061; AsyncStockSyncRateLimiter default + Retry-After.
...
- What changed:
  - `backend/app/services/marking_code_service.py`: регэксп `_CIS_CANDIDATE_RE` теперь допускает скобки `(01)`/`(21)` и пробелы/переносы строк вокруг обоих идентификаторов; добавлен `_canonical_cis_from_match()`, который всегда собирает код в каноническом «сыром» виде (`01<gtin>21<серийный>`) из именованных групп `gtin`/`serial`, а не берёт сырой текст совпадения (иначе в БД попадали бы скобки).
  - `backend/app/services/marking_label_artifact_service.py`: `_find_cis_boxes_on_page()` теперь строит список строк по ВСЕЙ странице (а не только внутри одного текстового блока) и, если код не находится в одной строке, пробует склеить её со следующей — так ловится код, разбитый на строки/блоки. Добавлена защита от ложного склеивания: если следующая строка уже сама по себе содержит полный код (как в обычных этикетках), склейка не выполняется — иначе подпись над кодом (например «Честный знак») «поглощала» бы соседний правильный код с неверной рамкой обрезки.
  - `backend/tests/test_marking_pdf_label_artifact.py`: добавлен regression-тест `test_pdf_import_parses_cis_wrapped_in_parens_across_two_lines` (воссоздаёт реальный формат этикетки с двумя строками и скобками, проверяет preview → import → артефакт этикетки).
- What did NOT change: формат хранения кода в БД (`cis_code`) — как и раньше, канонический `01<gtin>21<серийный>` без скобок; поведение для CSV/TXT-импорта и уже поддерживаемых «сырых» PDF-этикеток не тронуто (все прежние тесты зелёные).
- Verification (runtime proof):
  - Реальный файл из репорта (`~/Downloads/Chin-56005_beige_46_TGHU6649210_100pcs.pdf`, 100 страниц) прогнан через `extract_label_artifacts_from_pdf` напрямую: `real artifacts found: 100`, `unique cis: 100`, `unique gtin: {'04630321688296'}` (было 0 до фикса).
  - `cd backend && python3 -m pytest tests/ -k marking -q` → `104 passed`.
  - `cd backend && python3 -m pytest tests/test_marking_pdf_label_artifact.py -q` → `17 passed` (включая новый тест).
  - `cd backend && ruff check . && mypy .` → чисто на всём backend (270 файлов).
  - Полный `pytest` по всему backend не гонялся отдельно по явной просьбе владельца («срочно, не гонять весь CI») — точечная проверка (все тесты по маркировке + ruff/mypy на весь backend) пройдена перед push.
- Follow-up risk: регэксп покрывает конкретный формат этого продавца (скобки + разбиение на строки/блоки); если у других продавцов встретится ещё один непохожий способ печати кода — понадобится отдельный точечный фикс по тому же принципу (добавить регресс-тест на реальный файл).

## TASK-116 — 2026-07-10 — Деплой TASK-115 в prod + реальная загрузка остатков ИП Фадин

- Почему: код TASK-115 (импорт заявленного количества в сортировку) был готов и протестирован локально (366 backend + 105 frontend Playwright тестов зелёные), но не в git и не на сервере; продовые 42 товара ИП Фадин имели остаток 0.
- What changed:
  - Изменения TASK-115 закоммичены отдельной веткой `feat/tz-import-quantity-sorting-stock` (файлы печати этикеток/output из рабочей копии не тронуты), PR #101 с таблицей Test coverage (8 строк TC-, включая TC-NEW-TZI-01..03, TC-NEW-MP-AVAIL-01/02, TC-NEW-RACE-01, TC-NEW-UI-RACE-01).
  - CI на PR: `pr-quality-contracts`, `tc-coverage-report`, `backend` (366 passed), `e2e` (105 passed) — все зелёные; смержено в `main` (PR #101 → commit a49557c).
  - Автодеплой (`deploy.yml`) применил код на сервере (`194.87.96.144:8088`). Обнаружена гонка: `api` и `celery_worker` оба выполняют `alembic upgrade head` при старте; `celery_worker` выиграл гонку и создал таблицу `product_tz_imports` первым, `api` упал с `DuplicateTable` и перезапустился вручную (`docker compose up -d api`) — миграция `20260710_0060` в итоге применена один раз, `/api/health` → 200.
  - В prod через `apply_product_tz_import` (тот же сервисный код, что и HTTP-эндпоинт `/products/import-tz/apply`, вызван одноразовым скриптом внутри контейнера `api`, без обхода бизнес-логики и без прямых SQL-мутаций остатков) загружен реальный файл `ФФ Лыткарино (Фадин А.К.) (1).xlsx`: 42/42 товара обновлены (0 создано — все уже существовали), 0 ошибок, declared_total = 10 410. Остаток появился в зоне `__SORTING__` единственного склада тенанта.
  - Идемпотентность проверена в prod: повторный запуск того же файла вернул `skipped_count=42`, `updated_count=0` — остаток не задвоился (итоговая сумма осталась 10 410).
- What did NOT change: код фичи (см. TASK-115); ручное вмешательство в данные ограничено переносом seller_id (сделано ранее в этой же сессии) — новые остатки внесены только через протестированный сервисный путь.
- Verification (runtime proof):
  - `gh pr checks 101` → все 4 джобы `pass`.
  - SQL на prod до загрузки: `total_products=42, with_stock=0, total_qty=0`.
  - SQL на prod после загрузки: `total_products=42, with_stock=42, total_qty=10410`.
  - SQL на prod после повторного apply: `total_qty=10410` (не изменился).
  - `docker compose ps` на сервере: `api`, `celery_worker`, `celery_beat`, `db`, `redis`, `web` — все `Up`.
- Follow-up risk: одновременный `alembic upgrade head` в двух entry-point'ах (api + celery_worker) может повторно вызвать гонку на будущих релизах с новыми миграциями — стоит завести issue на «единая точка миграций» (например, миграции только в celery_worker/init-контейнере, api ждёт готовности).

## TASK-115 — 2026-07-10 — Excel ТЗ: заявленное количество в сортировку + единая доступность MP

- Почему: производственный Excel уже содержит колонку «Кол/во, заявленное клиентом», но импорт создавал/обновлял только карточки товара и не ставил заявленный остаток на склад. Из-за отдельной формулы общего остатка товар, находящийся только в зоне «Сортировка», также не показывался в подборе отгрузки на МП.
- What changed:
  - Импорт первого подходящего листа Excel читает необязательное заявленное количество, показывает его по строкам и суммой в preview, валидирует целое неотрицательное значение. Реальный файл `ФФ Лыткарино (Фадин А.К.) (1).xlsx` читается как 42 товарные строки с итогом 10 410; строка общего итога не считается товаром, копия листа повторно не обрабатывается.
  - При apply существующий товар того же селлера не дублируется, новый создаётся, а положительное количество добавляется движением `product_tz_import` в виртуальную ячейку `__SORTING__` единственного склада. Товары, движения, балансы и факт импорта атомарны.
  - Добавлена DB-идемпотентность по SHA-256 байтов файла и области tenant + seller + warehouse + import type. Повтор идентичного файла возвращает `already_applied`, не создаёт движений и не увеличивает остаток.
  - Все изменения balance buckets переведены на совместимую атомарную стратегию: приход — PostgreSQL/SQLite upsert, упаковка и списание — условные SQL updates. Параллельные import + package/deduct одного SKU/ячейки не теряют изменения, сохраняются `quantity = unpacked + packed` и отдельные движения.
  - Только конфликт именованного unique constraint импорта считается `already_applied`; прочие `IntegrityError` снова поднимаются как ошибки БД. Конкурентный повтор того же SHA добавляет остаток один раз; tenant/seller scopes проверены отдельно.
  - Seller и FF подборы товаров для отгрузки на МП используют отдельный readonly endpoint с формулой storage + sorting − outbound reserves − other MP reserves и строгой изоляцией селлера/тенанта. Канонические активные статусы (`submitted`, `confirmed`, `collecting`) вынесены в единый status contract; при `exclude_request_id` исключается только текущая заявка, второй активный резерв остаётся.
  - Поздние ответы detail/availability/preview/apply отбрасываются sequence guard после смены заявки, склада, селлера, файла или close/reopen. Во время apply селлер и файл заблокированы.
- What did NOT change: глобальный `inventory-balances/summary.available` остаётся storage-only; внутренний outbound и его резервы не менялись; picker склада в импорт не добавлялся; импорт по-прежнему берёт только первый лист с подходящей структурой.
- Verification: backend targeted 24/24 tests passed, full `ruff check .` and `mypy .` green; coverage includes true-overlap import+package/import+deduct, collecting plus second active MP reserve, exact IntegrityError classification, concurrent same-SHA and tenant/seller scope. PostgreSQL-marked overlap tests pass 2/2 on the SQLite-compatible path and are ready for isolated PostgreSQL through `WMS_TEST_DATABASE_URL`; live PostgreSQL/Alembic proof was not run because Docker daemon was unavailable (`Cannot connect to ... docker.sock`). Frontend changed-file ESLint, sequence-guard unit 2/2 and `npm run build` green. Full repository ESLint remains red on 32 pre-existing errors outside this task. Playwright: seller sorting-only picker + FF sorting-only picker + Excel preview/apply/apply-control-lock 3/3 passed. Production workbook read-only proof from the original slice: first matching sheet = 42 rows, declared total = 10 410. New Alembic transition `0059 → 0060` renders successfully; full empty-SQLite migration chain remains blocked by pre-existing migration `0005` (`ALTER CONSTRAINT` unsupported by SQLite).
- UI proof: в preview видны «Заявлено» и общий итог; отрицательное количество показывает понятную ошибку строки и блокирует «Применить»; в seller/FF «Добавить товары» sorting-only SKU виден с рассчитанным доступным количеством.
- Verifier race proof: Playwright 2/2 без sleeps — `TC-NEW-MP-AVAILABLE-002` удерживает detail+availability A, открывает B и после позднего release A сохраняет B/status/actions/stock; `TC-NEW-TZ-STOCK-002` удерживает apply и подтверждает disabled seller/file/Cancel, блокировку ESC и отсутствие уведомления до ответа.
- UX/UI contract: используется существующий диалог «Каталог → Загрузить Excel», без нового экрана и без выбора склада — при положительном количестве автоматически применяется единственный склад, иначе показывается ошибка. Preview показывает строки и общий заявленный итог; apply показывает success с добавленным количеством, duplicate без повторного прихода либо понятную row/apply error. Во время apply селлер, файл, Cancel и ESC заблокированы; внешнего UI-пути закрыть и заново открыть этот modal до завершения apply нет, поэтому late response не может попасть в новый экземпляр, а sequence guard остаётся защитой при программном размонтировании. MP picker визуально не переработан: изменён только источник и корректность доступного количества.
- Commit/deploy: не выполнялись по указанию владельца.

## TASK-114 — 2026-07-10 — Импорт товаров: лист по структуре (не по имени) + быстрое создание селлера

- Почему: 1) при загрузке товаров из Excel лист жёстко искался по имени, начинающемуся с «ТЗ Шаблон» — если продавец переименовывал/переносил лист (даже сохранив нужные колонки), импорт падал с «нет листа»; 2) не было быстрого способа создать нового селлера без ухода на отдельный экран и без обязательного email — просто чтобы сразу выбрать его при создании товара.
- What changed:
  - `backend/app/services/product_tz_import_service.py` — убрана привязка к имени листа (`TZ_SHEET_PREFIX`, `_pick_tz_sheet`); добавлена `_find_tz_sheet`, которая проходит по всем листам книги и берёт первый, чья структура колонок (шапка «Артикул продавца» + «Штрихкод»/«Информация для этикетки» в первых 30 строках) совпадает с шаблоном ТЗ. Имя листа теперь не проверяется вообще.
  - `backend/app/api/products.py` — убран мёртвый код ошибки `missing_sheet` (больше не выбрасывается).
  - `backend/tests/test_product_tz_import.py` — тест на старое поведение (`missing_sheet`) заменён тестами: имя листа не важно; лист выбирается по структуре среди нескольких листов в книге; если ни один лист не подходит по структуре — ошибка `missing_column`.
  - Новая кнопка «Создать селлера» на экране «Каталог» (`frontend/src/screens/v2/FfProductsCatalogScreen.tsx`) открывает `FfSellerCreateDialog` (новый файл, `frontend/src/screens/ff/FfSellerCreateDialog.tsx`) — только название, без email/пароля, вызывает уже существующий `POST /sellers`. После создания список селлеров обновляется во всём приложении (`onSellersChanged` → `refreshSellers` в `App.tsx`) без перезагрузки страницы, новый селлер сразу доступен в фильтре каталога и в выпадающем списке при создании товара — это тот же тип «Селлер», что и остальные в системе (без отдельного флага «ручной»/«из интеграции»).
- What did NOT change: экран «Селлеры» (`SellersScreen.tsx`, создание с email/логином) — оставлен как есть, это отдельный путь для селлеров с доступом в свой кабинет; модель `Seller` и backend-эндпоинт `POST /sellers` не менялись (использована существующая возможность); импорт коробок МП (`box_import_service.py`, другой формат) не тронут.
- Verification: backend — `ruff check .`, `mypy .`, `pytest` (352 passed, включая новые/обновлённые тесты импорта). Frontend — `npm run build` зелёный, `npx eslint` без новых ошибок. Playwright `tests-e2e/ff-products.spec.ts` — 5/5 passed, включая: обновлённый тест импорта Excel (лист теперь называется «Мой произвольный лист» вместо «ТЗ Шаблон (e2e)» — импорт всё равно находит колонки и создаёт товары) и новый тест `TC-NEW-SELLER-01` (создание селлера из каталога → сразу виден в фильтре и в диалоге создания товара → товар создаётся на этого селлера).

## TASK-113 — 2026-07-09 — Этикетка ШК: убрать «Размер», оставить цвет и бренд

- Почему: после фикса межстрочного интервала на 58×40 не влезали цвет/бренд — trim срезал их после «Размер: 0». Пользователь: размер не нужен, цвет и бренд — да.
- What changed: `productLabelDetailLines` больше не печатает размер; trim защищает строки «Цвет:»/«Бренд:»; чекбокс «Размер» убран из диалога печати; meta в одну строку (ellipsis).
- What did NOT change: размер в таблице ШК колонки каталога (`productBarcodeColumnSubLines`); line-height/ИП фикс TASK-112.
- Verification: unit 12 passed; симуляция очков — на 58×40 есть Цвет+Бренд, нет Размер/footer.
