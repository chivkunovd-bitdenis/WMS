...
def _assert_postgres_integer(value: int, message: str) -> int:
    if value < POSTGRES_INTEGER_MIN or value > POSTGRES_INTEGER_MAX:
...
async def create_tariff(
    ...
    *,
    ...
    if service_code == "storage_liter_day" and unit != "liter_day":
        raise BillingConfigurationError("Для хранения доступен только расчёт за литр-день")
    if service_code == "storage_liter_day" and warehouse_id is None:
        raise BillingConfigurationError("Для тарифа хранения укажите склад")
    if service_code != "storage_liter_day" and warehouse_id is not None:
        raise BillingConfigurationError("Склад можно указывать только для тарифа хранения")
    if seller_id is not None:
        await assert_seller_in_tenant(session, tenant_id=tenant_id, seller_id=seller_id)
    if warehouse_id is not None:
        ...
        if warehouse is None:
            raise BillingConfigurationError("Склад не найден в текущем tenant")
    amount_kopecks = _assert_postgres_integer(
        rub_to_kopecks(amount),
        "Ставка слишком велика: укажите сумму не больше 21 474 836,47 ₽",
    )
    ...
    # Validate every historical reprice before mutating the tariff stream.
    # PostgreSQL INTEGER is narrower than SQLite's integer, so this explicit
    # boundary prevents a production-only 500 and any partially repriced ledger.
    ...
    unpriced_query = unpriced_query.where(
        BillingLedgerEntry.warehouse_id == warehouse_id
    )
    unpriced = (await session.scalars(unpriced_query)).all()
    repricing: list[tuple[BillingLedgerEntry, Decimal, int]] = []
    for entry in unpriced:
        ...
        _assert_postgres_integer(
            repriced_amount,
            "Сумма дооценки слишком велика для сохранения; уменьшите ставку",
        )
        repricing.append((entry, quantity, repriced_amount))
    ...
    try:
        ...
        session.add(tariff)
        await session.flush()
        for entry, quantity, repriced_amount in repricing:
            entry.tariff_version_id = tariff.id
            entry.unit = unit
            entry.rate = amount_kopecks
            entry.quantity = quantity
            entry.amount = repriced_amount
        await session.flush()
