...
async def upsert_binding(
    ...
    *,
    ...
    row = FbsWarehouseBinding(
        ...
        is_active=True,
        # «Обслуживаем» — решение оператора, а не следствие сопоставления.
        # В модели у колонки стоит умолчание «включено» (миграция 28.08), и
        # из-за него любая новая привязка приезжала обслуживаемой: так у ИП
        # Фадина завелись восемь чужих складов, с которых потом приехали
        # заказы и 384 ложных списания. Новая привязка выключена, включает её
        # оператор галкой.
        served=False,
    )
    session.add(row)
