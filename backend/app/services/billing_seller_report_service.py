...
def moscow_interval(
    date_from: date, date_to: date, *, today: date | None = None
...
def _token(payload: dict[str, Any]) -> str:
    ...
    key = hmac.new(
        settings.jwt_secret_key.encode(), b"wms:seller-storage:v1", hashlib.sha256
    ).digest()
...
def _signed_cursor(payload: dict[str, Any]) -> str:
    ...
    key = hmac.new(
        settings.jwt_secret_key.encode(), b"wms:seller-report-cursor:v1", hashlib.sha256
    ).digest()
    signature = hmac.new(key, message, hashlib.sha256).hexdigest()
    return (
        base64.urlsafe_b64encode(
...
def _financial_totals(entries: list[dict[str, Any]]) -> dict[str, int]:
    gross = sum(
        _amount(row.get("amount_kopecks")) for row in entries if row.get("result") == "completed"
    )
    reversal = sum(
        abs(_amount(row.get("amount_kopecks")))
        for row in entries
        if row.get("result") == "reversed"
    )
    net = sum(
        _amount(row.get("amount_kopecks")) for row in entries if row.get("result") != "unpriced"
    )
    return {
    ...
    *,
    ...
    facts = list(
        (
            await session.scalars(
                facts_query.order_by(OperationFact.occurred_at.desc(), OperationFact.id.desc())
            )
        ).all()
    )
    ...
    if fact_ids:
        rows = await session.execute(
            ...
            .join(
                BillingLedgerLine, BillingLedgerLine.operation_fact_line_id == OperationFactLine.id
            )
            .join(BillingLedgerEntry, BillingLedgerEntry.id == BillingLedgerLine.ledger_entry_id)
            .where(
                BillingLedgerEntry.tenant_id == tenant_id,
                OperationFactLine.operation_fact_id.in_(fact_ids),
            )
        )
    ...
    if fact_ids:
        lines = await session.scalars(
            select(OperationFactLine)
            .where(OperationFactLine.operation_fact_id.in_(fact_ids))
    ...
    for fact in facts:
        ...
        product_names = [
            line.product_name_snapshot for line in product_lines if line.product_name_snapshot
        ]
        ...
        row: dict[str, Any] = {
            "id": f"operation_fact:{fact.id}",
            "kind": "operation_fact",
            "seller_id": str(fact.seller_id),
            "seller_name": fact.seller_name_snapshot or "Не указан",
            "occurred_at": _as_moscow(fact.occurred_at).isoformat(),
            "service_code": fact.billable_service_code or fact.operation_code,
            "item_quantity": fact.item_quantity,
            ...
            "result": "reversed"
            if fact.reversal_of_id
            else (
                "not_billable"
                if not fact.billable_service_code
                else (finance_result if include_finance else "completed")
            ),
        }
        if include_finance:
            row["rate_kopecks"] = (
                priced[0].rate if priced and len({entry.rate for entry in priced}) == 1 else None
            )
...
async def _legacy_entries(
    ...
    *,
    ...
    cutover = await _cutover(session)
    query = (
        select(BillingLedgerEntry, Seller.name)
        .outerjoin(Seller, Seller.id == BillingLedgerEntry.seller_id)
        .where(
            BillingLedgerEntry.tenant_id == tenant_id,
    ...
    rows = (
        await session.execute(
            query.order_by(BillingLedgerEntry.occurred_at.desc(), BillingLedgerEntry.id.desc())
        )
    ).all()
    ...
    if entry_ids:
        for line in (
            await session.scalars(
                select(BillingLedgerLine)
                .where(BillingLedgerLine.ledger_entry_id.in_(entry_ids))
                .order_by(BillingLedgerLine.id)
            )
        ).all():
    ...
    for entry, seller_name in rows:
        ...
        product_names = [
            ...
            for key in ("name", "product_name", "product_name_snapshot")
            if snapshot.get(key)
        ]
        skus = [
            str(snapshot[key])
            for snapshot in snapshots
            for key in ("sku", "sku_code", "sku_snapshot")
            if snapshot.get(key)
        ]
        finance_result = "unpriced" if entry.amount is None else "completed"
        row: dict[str, Any] = {
            "id": f"legacy_billing:{entry.id}",
            "kind": "legacy_billing",
            "seller_id": str(entry.seller_id),
        ...
        if include_finance:
            row.update(
                {
                    "unit": entry.unit,
                    "rate_kopecks": entry.rate,
                    "amount_kopecks": entry.amount,
...
async def _invoice_history(
    session: AsyncSession, *, tenant_id: uuid.UUID, seller_id: uuid.UUID, entry_id: str
) -> dict[str, Any]:
    invoices = list(
        (
            await session.scalars(
...
async def _storage_row(
    ...
    *,
        ...
        (
            await session.scalars(
                ...
                .where(
                    ...
                    InventoryMovement.seller_id == seller_id,
                    InventoryMovement.warehouse_id.in_(warehouse_ids or {uuid.UUID(int=0)}),
        ...
        for product in (
            await session.scalars(
                select(Product).where(
                    ...
                    Product.id.in_(product_ids or {uuid.UUID(int=0)}),
                )
            )
        ).all()
    }
    ...
    for product_id, product_movements in grouped.items():
        ...
        if product is None:
            continue
        calculated, product_missing = interval_liter_days(
            product_movements,
        ...
        fingerprint_sources.append(
            {
                ...
                "moves": [
                    (str(m.id), _as_moscow(m.created_at).isoformat(), m.quantity_delta)
                    for m in product_movements
                ],
                "dimensions": [
                    (str(e.id), _as_moscow(e.observed_at).isoformat(), str(e.volume_liters))
                    for e in events_by_product[product_id]
                ],
            }
        )
    tariff_rows = list(
        (
            await session.scalars(
                ...
                .where(
                    ...
                    BillingTariffVersion.valid_from <= date_to,
                )
    ...
    if not missing:
        for offset in range((date_to - date_from).days + 1):
            ...
            for product_id, product_movements in grouped.items():
                product = products.get(product_id)
                if product:
                    value, absent = interval_liter_days(
                        product_movements,
                        events_by_product[product_id],
                        legacy_volume_liters=product.volume_liters,
            ...
            effective = [
                ...
                if tariff.valid_from <= day and (tariff.valid_to is None or tariff.valid_to >= day)
            ]
            if effective:
                rate = effective[-1].amount
                amount += int(
                    (daily_liters * Decimal(rate)).quantize(Decimal("1"), rounding=ROUND_HALF_UP)
                )
    payload = {
        ...
        "liter_days": str(liter_days),
        "amount_kopecks": None if missing else amount,
    ...
    row: dict[str, Any] = {
        ...
        "date_from": date_from.isoformat(),
        "date_to": date_to.isoformat(),
        "liter_days": float(liter_days),
        "status": "missing_dimensions" if missing else "calculated",
        "calculation_token": _token(payload),
    }
...
async def build_seller_report(
    ...
    *,
    ...
    start, end = moscow_interval(date_from, date_to)
    entries = await _operation_entries(
        session,
        tenant_id=tenant_id,
        start=start,
        end=end,
        ...
        for row in entries:
            if row["kind"] == "legacy_billing":
                row["invoice_history"] = await _invoice_history(
                    ...
                    entry_id=row["billing_ledger_entry_id"],
                )
    sellers = list(
        (
            await session.scalars(
        ...
        rows.append(
            {
                ...
                **total,
                "details_target": f"/api/billing/seller-report/sellers/{seller.id}/details",
            }
        )
    totals = _totals(entries, include_finance=include_finance)
    return {
        "rows": rows,
        "totals": {"seller_count": len(rows), **totals},
        "entries": entries,
        "start": start,
        "end": end,
    }
