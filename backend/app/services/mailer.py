...
def send_email_sync(*, to: str, subject: str, body: str) -> bool:
    ...
    if not mail_configured():
        # Тело письма в лог не пишем. В нём лежит одноразовая ссылка со ссылочным
        # токеном, а по ней кто угодно задаёт пароль указанному человеку и сразу
        # получает доступ в его кабинет. Запрос сброса доступен без авторизации и
        # без ограничения частоты — значит посторонний мог бы налить в лог сколько
        # угодно рабочих ключей от чужих аккаунтов.
        logger.warning(
            "mail_not_configured: письмо не отправлено, to=%s subject=%s", to, subject
        )
        return False
