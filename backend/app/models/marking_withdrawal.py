...
class WithdrawalOperation(Base):
    ...
    workflow_lease_id: Mapped[uuid.UUID | None] = mapped_column(Uuid)
    workflow_lease_until: Mapped[datetime | None] = mapped_column(DateTime(timezone=True))
    workflow_error: Mapped[dict[str, Any] | None] = mapped_column(JSON)
    auth_signature_hash: Mapped[str | None] = mapped_column(String(64))
...
class WithdrawalItem(Base):
    ...
    preflight_evidence: Mapped[dict[str, Any] | None] = mapped_column(JSON)
