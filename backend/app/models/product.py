...
class Product(Base):
    ...
    # по направлениям в конкретных числах, и в процентную сетку она не ложится.
    # В этом режиме оператор задаёт неизменяемый событиями потолок: наружу уходит
    # min(потолок, свободный остаток), а число меняет только сам оператор.
