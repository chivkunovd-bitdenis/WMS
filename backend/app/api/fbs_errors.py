...
FBS_ERROR_MESSAGES_RU: dict[str, str] = {
    ...
    "scan_product_not_found": "Штрихкод товара не найден в текущей WB-поставке.",
    "scan_product_ambiguous": (
        "Штрихкод соответствует разным товарам или размерам. Печать не запущена."
    ),
    "scan_product_exhausted": (
        "Все подходящие единицы товара в этой поставке уже обработаны сканированием."
    ),
    "scan_auto_print_wb_only": "Автопечать по скану товара доступна только для WB FBS.",
    ...
    "scan_print_claim_not_owned": "Запуск печати уже закреплён за другой попыткой.",
    "not_a_kiz": "Код Честного знака не распознан. Отсканируйте полный КИЗ ещё раз.",
    "reprint_not_found": "Сохранённая перепечатка ЧЗ не найдена.",
    "print_claim_key_required": "Не передан идентификатор запуска печати.",
    "print_claim_key_too_long": "Идентификатор запуска печати слишком длинный.",
    "idempotency_key_required": "Не передан ключ идемпотентности.",
    "idempotency_key_too_long": "Ключ идемпотентности слишком длинный.",
    ...
    # Очередь печати на складской принтер (WMS-402).
    "print_job_not_found": "Задание печати не найдено.",
