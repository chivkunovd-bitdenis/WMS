...
def test_migrations_backfill_movements_and_exclude_technical_warehouses() -> None:
    ...
    movement_source = (
        versions / "20260822_0094_inventory_movement_reporting_dimensions.py"
    ).read_text()
    warehouse_source = (versions / "20260822_0097_storage_movement_scope.py").read_text()
    ...
    assert "name = 'FBS WB'" in warehouse_source
    assert "name LIKE 'FBS WB %'" in warehouse_source
    assert "code LIKE 'fbs-wb-%'" in warehouse_source
