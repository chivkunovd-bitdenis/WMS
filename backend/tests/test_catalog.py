...
@pytest.mark.asyncio
async def test_only_inventory_staff_can_update_product_dimensions(
    async_client: AsyncClient,
) -> None:
    ...
    assert reception_update.status_code == 403, reception_update.text
    assert reception_update.json()["detail"] == "forbidden"
    ...
    assert shift_lead_update.status_code == 403, shift_lead_update.text
    assert shift_lead_update.json()["detail"] == "forbidden"
