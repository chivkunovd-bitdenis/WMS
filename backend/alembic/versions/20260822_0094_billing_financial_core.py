...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column("settlement_account", sa.String(20), nullable=True),
        ...
        sa.UniqueConstraint("tenant_id", "seller_id", name="uq_billing_profiles_tenant_seller"),
    )
    ...
    op.create_index(
        "ix_billing_tariff_versions_tenant_id", "billing_tariff_versions", ["tenant_id"]
    )
    op.create_index(
        "ix_billing_tariff_versions_seller_id", "billing_tariff_versions", ["seller_id"]
    )
    op.create_index(
        "ix_billing_tariff_versions_warehouse_id", "billing_tariff_versions", ["warehouse_id"]
    )
    op.create_index(
    ...
    op.create_table(
        ...
        sa.Column("rate", sa.Numeric(28, 12), nullable=True),
        ...
        sa.CheckConstraint(
            "unit IN ('document', 'item', 'liter_day')", name="ck_billing_ledger_unit"
        ),
        ...
        sa.ForeignKeyConstraint(
            ["reversal_of_id"], ["billing_ledger_entries.id"], ondelete="RESTRICT"
        ),
    ...
    op.create_index(
        ...
        sqlite_where=sa.text("reversal_of_id IS NOT NULL"),
    )
    op.create_index(
        "ix_billing_ledger_tenant_occurred", "billing_ledger_entries", ["tenant_id", "occurred_at"]
    )
