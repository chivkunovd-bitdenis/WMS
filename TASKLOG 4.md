...
- What changed: `fbs_supply_validator_service.py` shared composition validator; `fbs_supply_reconcile_service.py` WB operation journal + reconcile; `fbs_supply_service.py` preflight/from-orders/start-work; API routes; `test_fbs_supply_from_orders.py` TC-01..06 (10 passed).
- What did NOT change: frontend; picking (→ FBSFLOW-050); print assets (→ FBSFLOW-080).
- Verification: ruff green; pytest **10 passed** 1 skipped on from-orders suite.
