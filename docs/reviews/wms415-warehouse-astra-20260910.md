...
- Финальный PG срез `test_wms156_discrepancy_concurrency.py test_inventory_counts.py
  ...
  Включены все три настоящих конкурирующих PG replay. Третий подтвердил ожидание
  удаления коробом stock-writer и отказ удаления после поступления в него3 единиц.
