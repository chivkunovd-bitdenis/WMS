...
async function main() {
    ...
    stdin: { contents: `import React,{useState} from 'react';
      ...
      function Harness(){const [open,setOpen]=useState(true);window.setDialogOpen=setOpen;
        ...
        return mounted && <Dialog open={open} onClose={()=>setOpen(false)} requestId={scope.request} boxId={scope.box}
          ...
          catalogById={catalog} warehouseStockByProductId={new Map()} onUpdated={async()=>{window.updatedScopes.push(scope.request)}}/>;}
  ...
  const results = [];
  try {
    const staleCases = ['stale-original', 'stale-retry', 'stale-json'];
    for (const scenario of baseline ? ['confirm'] : staleOnly ? staleCases : ['confirm', 'cancel-next', 'escape-next', 'close-pending', 'close-inflight', 'switch-request', 'switch-box', 'unmount', ...staleCases]) {
      const context = await browser.newContext();
      try {
        const calls = [], errors = [];
        let releaseA, releaseP, releaseRetry;
        const page = await context.newPage();
        if (scenario === 'stale-json') await page.addInitScript(() => {
          const originalFetch = window.fetch;
          window.fetch = async (...args) => {
            const response = await originalFetch(...args);
            if (String(args[0]).endsWith('/scan') && JSON.parse(args[1].body).allow_over_plan && response.ok) {
              const originalJson = response.json.bind(response);
              response.json = async () => {
                const body = await originalJson();
                window.scanJsonStarted = true;
                await new Promise(resolve => { window.releaseScanJson = resolve; });
                return body;
              };
            }
            return response;
        ...
        await context.route('**/*', async route => {
          ...
          if (body.barcode === 'P' && body.allow_over_plan && scenario === 'stale-retry') await new Promise(resolve => { releaseRetry = resolve; });
          ...
          if (!body.allow_over_plan && scenario !== 'stale-original') return route.fulfill({ status: 422, json: { detail: 'plan_limit_exceeded' } });
        ...
        if (staleCases.includes(scenario)) {
          if (scenario !== 'stale-original') {
            releaseP();
            await page.getByTestId('ff-mp-box-add-over-plan-confirm').click();
            if (scenario === 'stale-retry') await until(() => releaseRetry);
            else await page.waitForFunction(() => window.scanJsonStarted);
          }
          await page.evaluate(() => window.setDialogScope({request:'other',box:'other'}));
          if (scenario === 'stale-original') releaseP();
          else if (scenario === 'stale-retry') releaseRetry();
          else await page.evaluate(() => window.releaseScanJson());
          await pause(250);
          assert.deepEqual(await page.evaluate(() => window.updatedScopes), [], `${scenario}: obsolete callback must not refresh old parent document`);
          assert.equal(calls.length, scenario === 'stale-original' ? 2 : 3);
