-- WMS-081 / WMS-091 / WMS-096: только агрегаты, без cis_code, UUID,
-- названий селлеров, пользователей и других идентификаторов.
-- Скрипт не исправляет данные и принудительно запускается в read-only транзакции.

BEGIN TRANSACTION READ ONLY;

-- Общая форма состояний: сколько строк имеют время печати и товарную привязку.
SELECT
    status,
    source,
...
-- WMS-081: возраст действительно оставшихся reserved без вывода самих кодов.
SELECT
    CASE
...
-- WMS-091: инвариант новых печатных/применённых кодов, только счётчики.
SELECT
    status,
...
-- WMS-096: форма кандидатов recovery CLI. Выводятся только длины и признаки,
-- содержимое КИЗ и идентификаторы строк не выбираются.
SELECT
    status,
