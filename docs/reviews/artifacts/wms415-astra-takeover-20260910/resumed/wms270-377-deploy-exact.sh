...
test -z "$(git ls-files --others --exclude-standard -- backend/app frontend)"
...
test -z "$(git diff --name-only "$BASE_SHA" "$WMS270_TARGET_SHA" -- frontend backend/alembic docker-compose.prod.yml)"
...
curl --fail --silent --show-error --max-time 15 http://172.18.0.1:8088/seller/ > /dev/null
...
python3 scripts/deploy/verify-wms-host-network.py "$DB_CONTAINER"
for path in / /seller/ /api/health; do
  curl --fail --silent --show-error --max-time 15 "https://wms.sellerfocus.pro$path" > /dev/null
