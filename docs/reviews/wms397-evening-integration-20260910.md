...
## Фактическая выкладка на стейдж11.09.2026

По отдельному разрешению владельца «чат залейте на стейдж» выпущен source
`e3e6b1fa53bd5ab38394ebd1318b82aaf9e539ed` (runtime-код прежнего2a7a354d;
последующие commits меняли только документацию). Ветка отправлена до выпуска.
Railway проектloyal-wonder, внутреннее имя environment production относится
к стейджу WMS; VPS production не менялся. API710d8ef0-6db2-47b7-99af-eb5eeb2c0789
и webd06d7348-7802-4e6a-bd37-0961c1b72b25 получили SUCCESS.

До миграции выполнен приватный pg_dump stage и pg_restore --list; содержимое
не выводилось. База обновлена с0302 до20260911_0305.446 файлов backend/app
и backend/alembic внутри запущенного API совпали по SHA256 с Git. Публичные
HTMLFF/селлера и JS входовFF/селлера/ChatOpenButton совпали с runtime-web;
публичный /api/health вернулstatus=ok. Полные данные в artifacts/wms397-stage-20260911.json.

Адрес: https://web-production-9e7c1.up.railway.app/app/ff/chat . После reload
живой браузер показывает вход. Отправку/редактирование сообщений и вложения
в этой выкладке не проверяли без пользовательской сессии. Новые аккаунты,
пароли, сообщения клиентам и документы операторов не изменялись. Production
проверен по SSH: прежнийfb274e0d; чат туда не выложен. Фоновая задача не
воссоздавалась. Повторного общего ревью или локального полного набора тестов нет.
