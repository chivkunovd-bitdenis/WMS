...
> Детали постановок (если нужно глубже): `docs/CHESTNY_ZNAK_UX_FIXES_RU.md`, `docs/MASTER_BACKLOG_RU.md`.
...
2. **Соблюдать `depends_on`** — задача берётся в работу только когда у всех её зависимостей есть `.cursor/state/<id>.done`.
...
5. Каждая задача: минимальный diff, зелёная сборка/тест, строка в `TASKLOG.md`, коммит **в своём worktree** `.cursor/wt/<id>` (ветка `task/<id>`, база = HEAD **integration branch**).
...
| | | | **closed:** 2026-06-28 — файл в `docs/`, T-A7 = done (MP-022/023), таблица сверки с FIX_TASKS |
