...
- **Упаковка и задание на упаковку:** [`PACKAGING_RU.md`](PACKAGING_RU.md)
...
1. **Отдельный этап «упаковка» в жизненном цикле заявки на приёмку не вводим** — нет статусов «упакован / не упакован» у приёмки. Упаковка — **отдельный документ «Задание на упаковку»** и раздел **«Упаковка»** (см. [`PACKAGING_RU.md`](PACKAGING_RU.md)).
...
5. **ТЗ на упаковку товара** — заполняет **селлер в карточке товара**; обязательно перед отправкой заявки на отгрузку на ФФ; сотрудник ФФ видит инструкции **на каждой строке задания на упаковку** (и при необходимости дублирует в блоке отгрузки).
6. **Задание на упаковку и отгрузка на МП** — связь 1:1; задание **создаётся автоматически** при «Начата сборка»; проведение отгрузки **только после** выполненного задания; см. [`PACKAGING_RU.md`](PACKAGING_RU.md).
7. **Расходники при сборке отгрузки на МП** — на статусе «Начата сборка» можно (не обязательно) указать расходники с остатка **того же селлера**; при проведении отгрузки система **сохраняет расчёт** расхода (кол-во в отгрузке × норма на 1 шт.).
...
flowchart TB
  ...
  subgraph ff_in [Приёмка на ФФ]
    ...
    P0[Задание на упаковку — опционально]
  end
  subgraph ff_pack [Упаковка]
    P1[Раздел Упаковка / очередь заданий]
  end
  subgraph ff_out [Отгрузка на склад WB]
    O1[Состав отгрузки]
    O2[Начата сборка → авто-задание]
    O3[Подбор по ячейкам]
    O4[Упаковка — связанное задание]
    O5[Короба + скан + Отгружено]
  end
  S1 --> B1 --> B2
  B2 -.->|не сразу| B3
  B3 --> P0 --> P1
  S2 --> S3 --> O1 --> O2 --> O3 --> O4 --> O5
  O2 --> P1
  O4 --> P1
```
...
## P. Задание на упаковку

*Полное описание: [`PACKAGING_RU.md`](PACKAGING_RU.md).*

| ID | Статус | User story |
|----|--------|------------|
| **US-P-01** | Запланировано | **Как сотрудник ФФ**, я хочу раздел **«Упаковка»** с очередью заданий и возможностью **создать задание вручную** (место → товар → количество), чтобы упаковать товар **без отгрузки** (в т.ч. **часть** из «Сортировки»). |
| **US-P-02** | Запланировано | **Как сотрудник ФФ**, после поштучной приёмки / в «Сортировке» я хочу кнопку **«Упаковать»**, создающую **задание на упаковку** и открывающую форму работы. |
| **US-P-03** | Запланировано | **Как сотрудник ФФ**, в форме задания на **каждой строке** вижу **ТЗ на упаковку** из карточки товара (селлер). |
| **US-P-04** | Запланировано | **Как система**, остаток в месте делится на **не упаковано** и **упаковано**; выполнение задания переводит количество между ними **без смены ячейки**. |
| **US-P-05** | Запланировано | **Как сотрудник ФФ**, при **«Начата сборка»** отгрузки на МП система **автоматически создаёт одно связанное задание** (1:1); в отгрузке — ссылка и статус; из отгрузки кнопка **«Упаковка»** открывает **то же** задание (можно закрыть и продолжить позже). |
| **US-P-06** | Запланировано | **Как сотрудник ФФ**, в задании по отгрузке вижу по строкам: **сколько уже упаковано на полке** (из подбора), могу **подтвердить**; **нужно упаковать** = остаток; прогресс сохраняется при закрытии формы. |
| **US-P-07** | Запланировано | **Как система**, **провести отгрузку на МП нельзя**, пока связанное задание на упаковку **не выполнено** (все строки закрыты). |
| **US-P-08** | Запланировано | **Как сотрудник ФФ**, в задании, связанном с отгрузкой, в шапке — **ссылка на отгрузку** (как связанный документ в 1С). |
...
| ID | Статус | User story |
|----|--------|------------|
| **US-G-03** | Запланировано | **Как сотрудник ФФ**, при работе с **заданием на упаковку** (в т.ч. из отгрузки) вижу **ТЗ на упаковку** по каждому товару; дополнительно в документе отгрузки — сводный блок-ссылка на задание. |
...
| ID | Статус | User story |
|----|--------|------------|
| **US-G-05** | Запланировано | **Как сотрудник ФФ**, я хочу перевести отгрузку в статус **«Начата сборка»** — при этом **автоматически создаётся задание на упаковку** (US-P-05). |
...
| **P0** | US-P-04, US-P-01 … US-P-03 | Остаток упаковано/не упаковано + раздел «Упаковка» + ручное задание |
| **P0** | US-G-01, US-G-02, US-G-03, US-G-04 | ТЗ на упаковку в карточке товара |
| **P0** | US-P-05 … US-P-08, US-G-05 | Связь отгрузка ↔ задание, авто-создание при «Начата сборка», блок проведения |
| **P1** | US-G-06 … US-G-09 | Расходники на «Начата сборка», расчёт при проведении |
...
- Статус «упакован / не упакован» у **заявки на приёмку** (упаковка — отдельный документ, см. US-P).
