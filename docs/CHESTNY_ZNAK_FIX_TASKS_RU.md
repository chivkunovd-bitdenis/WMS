...
> **UX-бэклог (фронт):** [`docs/CHESTNY_ZNAK_UX_FIXES_RU.md`](./CHESTNY_ZNAK_UX_FIXES_RU.md) — отдельный трек T-*; таблица пересечений — в конце UX-дока (FINAL-03). Этот файл (CZ-B/H/M) **остаётся актуален** для бэкенда независимо от UX-задач.
