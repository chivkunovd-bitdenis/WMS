...
- Covered by e2e (TC mentioned in `frontend/tests-e2e/*.spec.ts`): 23/61
...
| `TC-S03-001` | Admin creates warehouse | Y | `catalog-journey.spec.ts` |
...
| `TC-S05-004` | Seller sees only allowed products | Y | `seller-cabinet.spec.ts` |
...
| `TC-S12-001` | Admin creates seller account bound to seller | Y | `seller-cabinet.spec.ts` |
| `TC-S12-002` | Seller login — dashboard shows seller context | Y | `seller-cabinet.spec.ts` |
| `TC-S12-003` | Seller sees filtered lists only for their seller | Y | `seller-cabinet.spec.ts` |
| `TC-S12-004` | Seller draft inbound/outbound within UI allowance | Y | `seller-cabinet.spec.ts` |
...
| `TC-S14-001` | Movements digest job lifecycle | Y | `sellers-and-background-job.spec.ts` |
