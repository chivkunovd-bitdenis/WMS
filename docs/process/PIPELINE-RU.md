...
Статус: **ACTIVE — единственный действующий процесс для новых задач WMS**.

Этот документ описывает один процесс для всей разработки WMS: от исправления текста или бага
до подключения нового маркетплейса целиком. Он является единственным нормативным источником
процесса. `AGENTS.md`, карточки правил и подсказки агентов содержат только краткое напоминание и
ссылку на версию этого документа, а не второй параллельный процесс.

Сам Markdown ничего не принуждает. Статус `ACTIVE` обеспечивают `pipeline/pipeline.yml`,
контроллер, CI-ворота и метатесты из части XII. Exact-SHA deploy остаётся отдельной стадией с
явным разрешением владельца и не запускается фактом активации процесса.
...
Статус `ACTIVE` означает, что Pipeline v2 является единственным обязательным каноном для всех
новых задач WMS и что переходы между стадиями принимает controller по машинному контракту.
Этот статус не является обещанием, что уже существует внешний распределённый хост или что агентам
даны production-доступы, registry credentials и независимый signing key. Эти возможности остаются
отдельными границами инфраструктуры и разрешений и не обходятся словом `ACTIVE`.

Новый процесс становится действующим только когда одновременно:

- E-1–E7 реализованы до управляемого repo/controller-контура и сохранены в Git;
- controller, CI и deploy fail-closed проверяют свои обязательные схемы, SHA и evidence;
- все объявленные метатесты зелёные;
- матрица E8 проверена метатестами и controller routing без ручной перестановки стадий;
...
Distributed host, внешний durable store, независимая криптографическая подпись receipt и удалённый
OCI registry развивают автономность и trust boundary, но не создают параллельный процесс. Пока их
нет, controller работает в управляемом режиме: карточки можно вести по стадиям, но нельзя обещать
полностью автономный ночной запуск на нескольких машинах или выполнять production-действия без
...
PIPELINE_ACTIVATION_APPROVED: 2026-08-21
```
