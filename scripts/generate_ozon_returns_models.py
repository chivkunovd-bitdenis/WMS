#!/usr/bin/env python3
"""Generate Pydantic v2 models from the checked-in Ozon Returns OpenAPI document.

The input is deliberately limited to OZON_RETURNS_OPENAPI.json.  Re-run this
script when the approved Ozon document is replaced; do not edit the generated
module by hand.
"""
...
OPENAPI_PATH = ROOT / "tasks/ozon-integration-20260825/OZON_RETURNS_OPENAPI.json"
OUTPUT_PATH = ROOT / "backend/app/schemas/ozon_returns_api.py"
...
def generate() -> str:
    ...
    enum_import = (
        ...
        if any("enum" in schema for schema in schemas.values())
    ...
    for schema_name, schema in schemas.items():
        ...
        lines.append(f"class {class_name}(OzonReturnsModel):")
        lines.append(f"    __openapi_name__ = {schema_name!r}")
        if description:
            lines.append(f"    {description!r}")
        additional = schema.get("additionalProperties")
    ...
    lines.append("")
    lines.append("__all__ = [*MODEL_BY_OPENAPI_NAME, 'MODEL_BY_OPENAPI_NAME', 'OzonReturnsModel']")
    lines.append("")
    return "\n".join(lines)
