...
ЭСКАЛАЦИОННЫХ_КРУГОВ = 1  # один финальный раунд сильным исполнителем
...
# находкам идёт Terra. Если два таких раунда не закрыли вердикт, даём ровно один
# финальный раунд Sol. После него обязательный вердикт запускается снова.
...
def _шаг(ид: str, роль: str, папка: Path, волна: Path,
    ...
    with контекст:
        for попытка in range(ПОВТОРОВ + 1):
            ...
            if профиль:
                доп["профиль"] = профиль
            код, хвост = запустить(роль, промпт(роль, ид, папка, волна, корень, рабочая) + дополнение,
                                   cwd=корень, **доп)
...
def активировать_эскалацию(папка: Path) -> Path:
    маркер = файл_эскалации(папка)
...
def провести(ид: str, волна: Path, рабочая: РабочаяКарточка | None = None) -> str:
    ...
    if круг > КРУГОВ + ЭСКАЛАЦИОННЫХ_КРУГОВ:
        журнал(волна, f"{ид}: отложено — финальная эскалация уже была")
        return "отложено"
    if круг > 0:
    ...
    while i < len(цепочка):
        ...
        if роль in ВОЗВРАЩАЮТ_К_DEV and есть_находки(папка, роль):
            ...
            мусор_для_повтора = ("DEV.md",)
            if роль == "ui-critic":
                мусор_для_повтора += ("CONTRACT.md", "CASES.md")
