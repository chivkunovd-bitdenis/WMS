...
# На первый заход разработчик дешёвый: код по готовому контракту и кейсам — работа
# посильная, а ошибку всё равно ловит ревью. Но переделка по находкам ревьюера — задача
...
def _шаг(ид: str, роль: str, папка: Path, волна: Path,
    ...
    for попытка in range(ПОВТОРОВ + 1):
        доп = {"модель": модель} if модель else {}
