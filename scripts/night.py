...
ЭСКАЛАЦИОННЫХ_КРУГОВ = 2  # два конечных Terra-ремонта; бесконечный цикл запрещён
МИНИМУМ_АГЕНТОВ = 6       # карточки не должны простаивать из-за занятого контроллера
МАКС_АТОМОВ = 6           # карточка не превращается в десятки последовательных запусков
МАКС_АТОМОВ_РЕМОНТА = 3  # находки ремонта группируются по слою и общим файлам
...
# Все штатные роли волны исполняет Codex. Claude остаётся только явным аварийным
# override через NIGHT_EXECUTOR=claude и не может включиться от отсутствия env.
ИСПОЛНИТЕЛЬ = os.environ.get("NIGHT_EXECUTOR", "codex").strip().lower()
...
def роль_с_инъекцией(роль: str, промпт: str, профиль: str | None = None) -> str:
    ...
    файл = КОРЕНЬ / ".claude" / "agents" / f"{роль}.md"
    инструкция = файл.read_text(encoding="utf-8", errors="replace") if файл.exists() else ""
    # `model:` в старом Claude-frontmatter нужен только для выбора профиля по умолчанию.
    # При явной эскалации он противоречил первой строке (`Sol`, но ниже `model: haiku`).
    инструкция = re.sub(r"^model:\s*(?:opus|sonnet|haiku)\s*$\n?", "", инструкция,
                        flags=re.M)
...
def артефакт_готов(папка: Path, роль: str) -> tuple[bool, str]:
    ...
    if роль == "splitter":
        ...
        ремонт = any(есть_находки(папка, роль_) for роль_ in
                     ("reviewer", "ui-critic", "ux-judge"))
        предел = МАКС_АТОМОВ_РЕМОНТА if ремонт else МАКС_АТОМОВ
        if len(куски) > предел:
            вид = "ремонт" if ремонт else "первичная нарезка"
            return False, (f"FEATURES.md: {вид} допускает не больше {предел} "
                           "связанных атомов; сгруппируй по слою и сценарию")
        if ремонт:
            владельцы: dict[str, int] = {}
            for номер, кусок in enumerate(куски, 1):
                секция_файлов = re.search(
                    r"^Файлы\s*:?\s*(.*?)(?=^(?:Зависит|Зависимости|Как проверить|Проверка)\s*:?|\Z)",
                    кусок, re.M | re.S | re.I)
                файлы = set(re.findall(
                    r"(?:backend|frontend)/[A-Za-z0-9_./-]+",
                    секция_файлов.group(1) if секция_файлов else ""))
                for путь in файлы:
                    if путь in владельцы:
                        return False, (f"FEATURES.md: файл {путь} раздроблен между атомами "
                                       f"{владельцы[путь]} и {номер}; собери в один")
                    владельцы[путь] = номер
    ...
    if роль == "ux-architect":
        ...
        if видимый_фронтовый_баг(папка) and not макет_нужен:
            return False, "видимому frontend-багу обязателен MOCKUP.html"
        if макет_нужен:
...
def браузерный_блокер(папка: Path, роль: str) -> bool:
    """Не отправлять код на переделку, если живой экран вообще не открылся."""
    if роль not in ("clicker", "ux-judge"):
        return False
    файл = папка / ("CLICKS.md" if роль == "clicker" else "JUDGE.md")
    if not файл.exists():
        return False
    ...
    нет_среды = any(причина in текст for причина in (
        ...
        "нет браузерного сеанса",
    ))
    нет_живого_экрана = any(признак in текст for признак in (
        "живой экран не был показан", "ни одного снимка", "снимок отсутствует",
    ))
    # Старые ux-judge иногда ошибочно писали `ВЕРДИКТ: НАХОДКИ`, хотя прямо
    # подтверждали, что экран вообще не открывался. Такой отчёт не является code finding.
    экран_не_видели = any(признак in текст for признак in (
        "не сделал ни одного снимка", "проверить физически невозможно",
        "не открыл экран", "не открыл ни одного экрана", "не удалось открыть экран",
    ))
    return (нет_среды and (заблокировано or экран_не_видели)
            or заблокировано and нет_живого_экрана)


def браузерная_парковка(папка: Path) -> bool:
    файл = папка / "OTLOZHENO.md"
    if not файл.exists():
        return False
    текст = файл.read_text(encoding="utf-8", errors="replace").lower()
    return any(признак in текст for признак in (
        "браузерная среда недоступна", "стенд не поднялся", "живой экран"))
...
def снять_старую_браузерную_парковку(папка: Path) -> bool:
    ...
    парковка.unlink(missing_ok=True)
    return True
...
def видимый_фронтовый_баг(папка: Path) -> bool:
    """Баг меняет живой экран: ему нужны UX-контракт и breaker."""
    if тип_карточки(папка) != "баг":
        return False
    файл = папка / "RAZBOR.md"
    разбор = файл.read_text(encoding="utf-8", errors="replace") if файл.exists() else ""
    return bool(re.search(r"\bS-\d\d\b|(?:^|[ `/])frontend/", разбор, re.M | re.I))


def цепочка_карточки(папка: Path, тип: str) -> list[str]:
    """Бэкенд-баг остаётся коротким; видимый баг не прыгает через UX."""
    if тип == "баг" and видимый_фронтовый_баг(папка):
        return ["product", "ux-architect", "tester", "breaker", "splitter", "dev",
                "reviewer", "ui-critic", "clicker", "ux-judge"]
    return list(ЦЕПОЧКИ[тип])


def первый_непройденный(папка: Path, цепочка: list[str], до: int) -> int | None:
    """Валидирует prerequisite-prefix перед точкой resume."""
    for номер, роль in enumerate(цепочка[:до]):
        проверяемая = "backend-dev" if роль == "dev" else роль
        готово, _ = артефакт_готов(папка, проверяемая)
        if not готово or (роль in РОЛИ_С_ВЕРДИКТОМ and есть_находки(папка, роль)):
            return номер
    return None
...
def дифф_для(роль: str, рабочая: "РабочаяКарточка | None") -> str:
    ...
    return ("\n\nПравка продукта в этой карточке — ровно эти файлы:\n" + список +
            ...
            "К границам экрана они отношения не имеют и находкой быть не могут. Сверь каждый "
            "продуктовый файл с allowlist атомов FEATURES.md; лишний экран, UI-kit, route, "
            "кнопка или соседний сценарий — блокирующая находка.")
...
# Всю продуктовую разработку делает Terra: и первичный атом, и ремонт по находкам.
# Sol остаётся независимым аналитиком/ревьюером и не расходуется на написание кода.
МОДЕЛЬ_ПЕРЕДЕЛКИ: dict[str, tuple[str, str]] = {
    "backend-dev": ("sonnet", "terra"),
    "screen-dev": ("sonnet", "terra"),
    "ux-architect": ("sonnet", "terra"),
}
МОДЕЛЬ_ЭСКАЛАЦИИ = ("sonnet", "terra")


def шаг(*args, круг: int = 0, **kwargs):
    """Обёртка: на переделке поднимает модель разработчика."""
    if args[1] in ("backend-dev", "screen-dev"):
        kwargs["модель"], kwargs["профиль"] = МОДЕЛЬ_ПЕРЕДЕЛКИ[args[1]]
    elif круг > КРУГОВ and args[1] in МОДЕЛЬ_ПЕРЕДЕЛКИ:
...
def файлы_атома(атом: str) -> set[str]:
    """Машинный allowlist атома: ни одного соседнего домена «заодно»."""
    return set(re.findall(
        r"(?:backend|frontend|mobile|tests)/[A-Za-z0-9_./-]+", атом,
    ))


def _грязные_пути(корень: Path) -> set[str]:
    """Все изменённые, staged и untracked пути без хрупкого парсинга status."""
    пути: set[str] = set()
    for аргументы in (("diff", "--name-only"), ("diff", "--cached", "--name-only"),
                       ("ls-files", "--others", "--exclude-standard")):
        р = _git(*аргументы, cwd=корень)
        if р.returncode == 0:
            пути.update(строка.strip() for строка in р.stdout.splitlines() if строка.strip())
    return пути


def ошибка_scope(пути: set[str], разрешено: set[str]) -> str:
    """Лишний продуктовый файл — машинный стоп, а не вкус reviewer-а."""
    лишние = sorted(путь for путь in пути
                     if not путь.startswith(СТАДИЙНЫЕ) and путь not in разрешено)
    if not лишние:
        return ""
    особо_опасные = [п for п in лишние if re.search(
        r"frontend/src/(?:ui-kit|.*(?:route|screen))", п, re.I)]
    хвост = ("; отдельно запрещены новые UI-kit/route/screen: "
             + ", ".join(особо_опасные)) if особо_опасные else ""
    return "выход за scope атома: " + ", ".join(лишние) + хвост
...
def подготовить_зависимости(
        волна: Path, рабочие: dict[str, "РабочаяКарточка"], *,
        точечный_resume: bool = False) -> list[str]:
    ...
    ремонтные = {ид for ид, рабочая in рабочие.items()
                 if (круг_из_парковки(рабочая.папка) > 0
                     or переплан_уже_был(рабочая.папка)
                     or браузерный_блокер(рабочая.папка, "ux-judge")
                     or браузерная_парковка(рабочая.папка))}
    ...
    if точечный_resume:
        # OWNER-SCOPE restart намеренно снимает старую FEATURES.md и возвращает
        # карточку в ux-architect. Номера DEPENDENCIES.json относятся к прежней
        # нарезке и до нового splitter не имеют смысла. Не валидируем из-за них
        # чужие milestones и не блокируем обязательный prefix карточки.
        активные_требования = {
            цель: нужны for цель, нужны in активные_требования.items()
            if артефакт_готов(
                рабочие[цель.split("#", 1)[0]].папка, "splitter")[0]
        }
...
def сохранить_checkpoint(рабочая: РабочаяКарточка, сообщение: str,
                         атом: str | None = None) -> tuple[bool, str]:
    ...
    пути = _грязные_пути(рабочая.корень)
    ...
    разрешённые_продукта = файлы_атома(атом or "")
    if беда := ошибка_scope(пути, разрешённые_продукта):
        return False, беда
    папка_карточки = os.path.relpath(
        рабочая.папка, рабочая.корень).rstrip("/")
    служебные = {п for п in пути
                   if п.startswith(папка_карточки + "/")
                   and re.search(r"/DEV(?:-\d\d)?\.md$", п)}
    для_коммита = sorted((пути & разрешённые_продукта) | служебные)
    уже_в_index = set(_git(
        "diff", "--cached", "--name-only", cwd=рабочая.корень).stdout.splitlines())
    if лишние_в_index := sorted(уже_в_index - set(для_коммита)):
        return False, "в index уже есть чужие файлы: " + ", ".join(лишние_в_index)
    if not для_коммита:
        return True, "безопасных изменений для checkpoint нет"
    добавить = _git("add", "--", *для_коммита, cwd=рабочая.корень)
...
def разработать_по_фичам(ид: str, папка: Path, волна: Path,
    ...
    for позиция, (номер, кусок) in enumerate(выбранные, 1):
        ...
        дополнение += (
            ...
            " Менять можно только файлы, прямо названные в этом атоме. Новые UI-kit "
            "компоненты, routes, screens, соседние кнопки и сценарии запрещены, если их нет в allowlist атома."
        )
        if источник_находок:
            дополнение += (
            ...
            if позиция == len(выбранные):
                дополнение += (
                    "\n\nЭТО ПОСЛЕДНИЙ АТОМ РЕМОНТА. Перед сдачей прочитай все DEV-NN.md "
                    "этого ремонта и один раз запусти совокупный целевой набор тестов из "
                    "всех атомов и замороженного вердикта. Не запускай полный pytest или "
                    "весь E2E. Красную целевую интеграцию исправь в границах ремонта до "
                    "повторного reviewer: reviewer не должен быть первым интеграционным тестом.")
        ...
        сохранено, сведения = сохранить_checkpoint(
            рабочая, f"night({ид}): atom {номер}/{len(куски)}", атом=кусок)
        if not сохранено:
...
def ремонтный_план_устарел(папка: Path) -> bool:
    """Новый отрицательный вердикт ещё не превращён splitter-ом в FEATURES."""
    источник = отрицательный_вердикт(папка)
    план = папка / "FEATURES.md"
    return источник is not None and (
        not план.exists() or план.stat().st_mtime <= источник[1].stat().st_mtime)


def перепланировать_ремонт(ид: str, папка: Path, волна: Path,
                          рабочая: РабочаяКарточка,
                          круг: int = КРУГОВ) -> tuple[bool, str]:
    """Дорезает актуальный отрицательный вердикт до ремонтных атомов."""
    источник = отрицательный_вердикт(папка)
    if источник is None:
    ...
    ок, беда = шаг(
        ...
        дополнение=(
            ...
            "FEATURES.md только из ещё не закрытых находок. Максимум три атома: группируй "
            "по слою и связанному сценарию. Все находки одного файла обязаны попасть в один "
            "атом и к одному исполнителю. Один-три продуктовых файла на атом, отдельная проверка. "
...
def аннулировать_после_вердикта(папка: Path, роль: str) -> None:
    ...
    if роль == "reviewer":
        ...
        удалить.update(("REVIEW.md", "JUDGE.md"))
    elif роль == "ux-judge":
...
def провести(ид: str, волна: Path, рабочая: РабочаяКарточка | None = None) -> str:
    ...
    папка = рабочая.папка
    браузерный_resume = снять_старую_браузерную_парковку(папка)
    ...
    цепочка = цепочка_карточки(папка, тип)
    scope_до_resume = set(дифф_реализации(
        рабочая.корень, рабочая.base_sha))
    if круг > 0 and ремонтный_план_устарел(папка):
        # Старые циклы по исходным атомам не были попытками исправить актуальный
        # вердикт. Даём ему один нормальный Sol-ремонт вместо немедленной парковки.
        круг = min(круг, КРУГОВ + 1)
        журнал(волна, f"  {ид} · repair-plan: новый вердикт дорезается до dev")
        ок, беда = перепланировать_ремонт(ид, папка, волна, рабочая, круг)
        if not ок:
            (папка / "OTLOZHENO.md").write_text(
                f"перепланирование ремонта: {беда}\n", encoding="utf-8")
            журнал(волна, f"{ид}: отложено — не удалось перенарезать актуальный вердикт")
            return "отложено"
    ...
    if круг > 0:
        ...
        i = цепочка.index("clicker") if браузерный_resume else 0
        if браузерный_resume:
            журнал(волна, f"  {ид} · resume: повторяем clicker после недоступного стенда")
    # Resume не имеет права прыгнуть через потерянную обязательную стадию.
    если_пропущен = первый_непройденный(папка, цепочка, i)
    if если_пропущен is not None:
        i = если_пропущен
        круг = 0
        журнал(волна, f"  {ид} · resume: восстанавливаем обязательный шаг {цепочка[i]}")
    while i < len(цепочка):
        if цепочка[i] in ("reviewer", "ui-critic", "clicker", "ux-judge"):
            разрешено = scope_до_resume | set().union(
                *(файлы_атома(атом) for атом in фичи(папка)))
            текущий_scope = set(дифф_реализации(
                рабочая.корень, рабочая.base_sha))
            if беда := ошибка_scope(текущий_scope, разрешено):
                (папка / "OTLOZHENO.md").write_text(беда + "\n", encoding="utf-8")
                журнал(волна, f"{ид}: отложено — scope gate перед {цепочка[i]}")
                return "отложено"
        if цепочка[i] == "dev":
            роль = "dev"
        ...
        if браузерный_блокер(папка, роль):
            (папка / "OTLOZHENO.md").write_text(
                f"{роль}: браузерная среда недоступна; код на переделку не возвращался\n",
                encoding="utf-8",
            )
        ...
        if роль in ВОЗВРАЩАЮТ_К_DEV and есть_находки(папка, роль):
            ...
            if круг > КРУГОВ:
                активировать_эскалацию(папка, круг)
                журнал(волна, f"  {ид} · {роль}: находки не закрыты — повторный Terra-ремонт")
            журнал(волна, f"  {ид} · {роль}: находки, круг {круг} — назад к разработке")
            # Исходные атомы описывают первоначальную разработку. Находка может затронуть
            # другой файл того же сценария; слепое сопоставление со старым FEATURES тогда
            # гоняет разработчиков по нерелевантным атомам без изменения кода. Поэтому
            # каждый отрицательный вердикт сначала превращаем в короткий repair-план.
            перепланирован, причина = перепланировать_ремонт(
                ид, папка, волна, рабочая, круг)
            if not перепланирован:
                (папка / "OTLOZHENO.md").write_text(
                    f"перепланирование ремонта: {причина}\n", encoding="utf-8")
                журнал(волна, f"{ид}: отложено — не удалось перенарезать находки")
                return "отложено"
            i = цепочка.index("dev")
            continue
        i += 1
...
def ночь(волна: Path, полос: int, фильтр: str | None = None) -> int:
    ...
    ошибки_зависимостей = подготовить_зависимости(
        волна, рабочие, точечный_resume=bool(фильтр))
