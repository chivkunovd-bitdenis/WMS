...
## ⛔️⛔️⛔️ СНАЧАЛА ИССЛЕДУЙ ВЕСЬ ПРОЦЕСС. ПОТОМ ПРЕДЛАГАЙ РЕШЕНИЕ И МЕНЯЙ КОД ⛔️⛔️⛔️

Перед любой реализацией сначала глубоко исследуй затронутый процесс. Не
останавливайся на первой найденной функции, условии или правдоподобной причине.
Проследи задачу от действия оператора или входящего события через интерфейс,
API, бизнес-логику, изменения состояния и данных, фоновые задачи и внешние
системы — до конечного результата и того, что после повторного чтения увидит
оператор.

Для изменяемого участка обязательно:

- прочитай текущую реализацию и последние изменения тех же файлов и функций;
- найди все места в этой цепочке, которые могут изменить маршрут, запретить
  действие, вернуть процесс назад или повлиять на соседний документ;
- проследи все записи и последующие чтения затронутых данных;
- проверь применимые состояния: обычное и частичное выполнение, уже завершённое
  действие, повтор, ошибка, отмена и незавершённая операция;
- проверь связанные процессы и активные ветки, если они затрагивают те же файлы
  или могут вернуть старую логику.

**До первой правки обязательно напиши пользователю:**

1. как понял задачу и какое конкретное решение предлагаешь;
2. какую цепочку процесса и какие конкретные участки кода исследовал;
3. с какими экранами, документами, операциями и внешними системами это связано;
4. какие состояния и последствия проверил;
5. что может сломаться, заблокировать оператора или изменить данные;
6. что осталось непроверенным и почему.

Фразы «изучил фронт и бэк», «проверил зависимости» и «рисков нет» без конкретной
цепочки, функций и состояний не считаются исследованием. Если цепочка ещё не
исследована до конца, не начинай реализацию и не выдавай предположение за
решение.

Это не отдельный согласовательный гейт: после такого сообщения продолжай работу
самостоятельно. Остановись и запроси решение владельца только тогда, когда после
исследования обнаружено отсутствующее решение, которое действительно меняет
продуктовый смысл или объём поставленной задачи.
