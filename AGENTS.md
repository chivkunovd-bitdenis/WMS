...
## ⛔️ ПРАВИЛЬНЫЕ ПУТИ ПРОЕКТА — ЧИТАТЬ ПЕРВЫМ

```
/Users/deniscivkunov/Projects/WMS
/Users/deniscivkunov/Projects/WMS/.worktrees/<зарегистрированный-git-worktree>
```

Перед первой правкой проверь `pwd`: разрешён канонический корень или зарегистрированный
`git worktree` внутри его `.worktrees/`. Если путь не относится ни к одному из них —
**остановись и скажи пользователю**, ничего не правь.
...
Для затронутого экрана UX до Atomic Dev обязан сохранить открываемый `MOCKUP.html`,
собранный по существующему ui-kit и показывающий все предусмотренные состояния. Если
макету нужен новый общий элемент, он описывается отдельно и реализуется с тестом в
`frontend/src/ui-kit/` раньше экранного атома. Code Review и Browser Product QA сверяют
живой результат не только с текстовым контрактом, но и с этим макетом.
