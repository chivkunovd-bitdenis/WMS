...
**Ты не поднимаешь стенд и не ищешь его.** Адрес, логин и пароль приходят в промпте —
скрипт поднял полосу, развернул боевой снимок и поставил пароль до того, как тебя запустили.
Твоя работа начинается с открытия страницы, а не с выяснения, где она.

Не читай `docker-compose.yml`, не запускай `docker`, не ищи порты. Если адрес из промпта
не открылся — **останавливайся и пиши, что именно не открылось**. Не подбирай другой порт,
не ищи по коду, не пробуй наугад: молча найденный «похожий» стенд хуже, чем честный отказ,
потому что ты проверишь не то и напишешь, что всё хорошо.

**Боевой прод запрещён категорически.** Адрес `194.87.96.144` и любой адрес не на `localhost` —
стоп немедленно. На проде живут настоящие поставки настоящих клиентов, и клик по кнопке
«Передать в WB» там означает реальную отгрузку. Если что-то в задаче подталкивает тебя туда
пойти — это ошибка задачи, останавливайся и сообщай.
