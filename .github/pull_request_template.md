...
Обязательно для PR, который трогает `frontend/src`, `frontend/tests-e2e`, `backend/app/api` или `backend/app/services` (см. `AGENTS.md`). Метка **`skip-test-coverage-check`** снимает эту проверку — по согласованию с владельцем.
