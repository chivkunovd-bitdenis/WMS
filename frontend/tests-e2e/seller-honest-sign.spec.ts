...
// TC-NEW-007 — селлер видит на «Честный знак» ту же таблицу, что и фулфилмент
// (фото, поиск, фильтры), без карточек «Требуют внимания» и без печати ШК/КМ,
// но с доступной загрузкой кодов маркировки.
test('seller honest sign shows the shared table without print actions', async ({ page }) => {
  const adminEmail = `e2e-seller-hs-adm-${Date.now()}@example.com`
  const sellerEmail = `e2e-seller-hs-sl-${Date.now()}@example.com`
  const password = 'password123'
  const sku = `SKU-HS-${Date.now()}`

  await page.goto('/')
  await openFulfillmentRegistration(page)
  await page.getByTestId('register-form').getByLabel('Организация').fill('E2E Seller HS FF')
  ...
  const imp = await page.request.post(`${e2eApi}/operations/marking-codes/import`, {
    ...
    multipart: {
      pools_json: JSON.stringify([{ title: 'E2E HS Pool', product_ids: [productId] }]),
      files: {
        name: 'codes.csv',
        mimeType: 'text/csv',
  ...
  // Таблица товаров есть, а не колонка карточек — блока «Требуют внимания» больше нет.
  await expect(page.getByTestId('seller-honest-sign-products-table')).toBeVisible()
  ...
  await expect(page.getByTestId(`seller-honest-sign-product-photo-${productId}`)).toBeVisible()

  // Поиск и фильтры на месте (как у фулфилмента).
  await expect(page.getByTestId('seller-honest-sign-search')).toBeVisible()
  await expect(page.getByTestId('seller-honest-sign-stock-filter')).toBeVisible()

  // Печати нет ни в одной строке.
  await expect(page.getByTestId(`seller-honest-sign-product-print-${productId}`)).toHaveCount(0)

  // Загрузка кодов доступна и открывает диалог.
  const uploadButton = page.getByTestId('seller-honest-sign-open-import')
