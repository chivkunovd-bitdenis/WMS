...
const rows = [
  ...
  { id: 'draft-ready', seller_id: 'seller-3', seller_name: 'Норд', warehouse_id: 'warehouse-1', warehouse_name: 'Основной склад', status: 'draft', fixed_at: null, total_liter_days: '6432.00', total_amount: '4502.40', problem_count: 0, measurements: [{ product_id: 'product-nord', sku: 'SKU-20001-VERY-LONG-UNIQUE-SUFFIX', seller_article: 'NRD-READY', volume_liters: '1.20', dimensions_source: 'wildberries', liter_days: '6432.00', rate_snapshot: '0.70', amount: '4502.40', status: 'calculated' }] },
...
test('S-11-TC-007 keeps history columns visible with a long author name', async ({ page }) => {
  await openStorage(page)
  await page.getByTestId('storage-expand-draft-problem').click()
  await page.getByTestId('storage-history-product-ready').click()
  ...
  const historyTable = page.getByTestId('storage-history-table')
  const [historyTableBox, historyOverflow, authorHeaderBox, appliedHeaderBox, authorCellBox, appliedCellBox] = await Promise.all([
    historyTable.boundingBox(),
    historyTable.evaluate((element) => ({ clientWidth: element.clientWidth, scrollWidth: element.scrollWidth })),
...
test('S-11-TC-009 keeps rate and amount visible beside a long seller article in print preview', async ({ page }) => {
  ...
  const table = page.getByTestId('storage-print-table')
  const [tableBox, tableOverflow, rateHeaderBox, amountHeaderBox, rateCellBox, amountCellBox] = await Promise.all([
    table.boundingBox(),
    table.evaluate((element) => ({ clientWidth: element.clientWidth, scrollWidth: element.scrollWidth })),
