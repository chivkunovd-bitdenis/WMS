...
// TC-NEW-011 — CROSS-02: бейдж на упаковке = число строк в pending-marking (единый контракт total/rows).
test('FF packaging pending badge matches pending-marking row count', async ({ page }) => {
  test.setTimeout(180_000)
  const email = `e2e-pending-badge-${Date.now()}@example.com`
  const password = 'password123'
  const e2eApi = process.env.E2E_API_ORIGIN ?? 'http://127.0.0.1:18000'
  const ts = Date.now()
  ...
  await page.goto('/')
  await openFulfillmentRegistration(page)
  await page.getByTestId('register-form').getByLabel('Организация').fill('E2E Pending Badge')
  await page.getByTestId('register-form').getByLabel('Email администратора').fill(email)
  await page.getByTestId('register-form').getByLabel('Пароль').fill(password)
  const [regRes] = await Promise.all([
    waitForPostOk(page, '/api/auth/register'),
  ...
  const sellerRes = await page.request.post(`${e2eApi}/sellers`, {
    headers: auth,
    data: JSON.stringify({ name: 'E2E Badge Seller', email: `s-badge-${ts}@example.com` }),
  })
  const sellerId = String(((await sellerRes.json()) as { id: string }).id)
  const whRes = await page.request.post(`${e2eApi}/warehouses`, {
    headers: auth,
    data: JSON.stringify({ name: 'WH Badge', code: `wh-badge-${ts}` }),
  })
  const whId = String(((await whRes.json()) as { id: string }).id)

  const productSpecs = [
    { name: 'E2E Badge A', sku: `SKU-BG-A-${ts}`, gtin: '000000003333' },
    { name: 'E2E Badge B', sku: `SKU-BG-B-${ts}`, gtin: '000000004444' },
  ] as const
  ...
  for (const spec of productSpecs) {
    ...
    const cis = `01${spec.gtin}21${'Y'.repeat(20)}0001`
    await page.request.post(`${e2eApi}/operations/marking-codes/import`, {
      headers: bearer,
      multipart: {
        seller_id: sellerId,
  ...
  await page.getByTestId('ff-packaging-create-open').click()
  await page.getByTestId('ff-packaging-create-warehouse').click()
  await page.getByRole('option', { name: 'WH Badge' }).click()
  await Promise.all([
    page.waitForResponse(
      (r) =>
        r.request().method() === 'POST' &&
