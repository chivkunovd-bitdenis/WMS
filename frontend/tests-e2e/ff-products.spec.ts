...
// Given: FF admin, селлер, xlsx с объединённым ТЗ; When: «Загрузить Excel» и Применить;
// Then: товары в каталоге, ТЗ заполнено, бейдж «Вручную».
...
wb = Workbook()
ws = wb.active
ws.title = "ТЗ Шаблон (e2e)"
ws.append(["Артикул продавца","Фото","Размер","Штрихкод","Информация для этикетки","Пожелания/Инструкция по обработке, упаковке и фасовке"])
ws.append(["E2E-ART", None, 46, None, "2039000000001", None])
ws.append(["E2E-ART", None, 48, None, "2039000000002", None])
ws["F2"] = "E2E merged TZ"
...
`
  ...
  await Promise.all([
    ...
    page.getByTestId('ff-tz-import-apply').click(),
  ])
