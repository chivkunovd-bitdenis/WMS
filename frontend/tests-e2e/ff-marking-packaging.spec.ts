...
test('FF packaging: print honest sign codes for line quantity', async ({ page }) => {
  ...
  await page.getByTestId('marking-print-wb-qty').locator('input').fill('1')
...
test('FF packaging: reprint selected marking codes', async ({ page }) => {
  ...
  await page.goto('/')
  await openFulfillmentRegistration(page)
  await page.getByTestId('register-form').getByLabel('Организация').fill('E2E Reprint1')
  await page.getByTestId('register-form').getByLabel('Email администратора').fill(email)
  await page.getByTestId('register-form').getByLabel('Пароль').fill(password)
  const [regRes] = await Promise.all([
    waitForPostOk(page, '/api/auth/register'),
  ...
  const sellerRes = await page.request.post(`${e2eApi}/sellers`, {
    headers: auth,
    data: JSON.stringify({ name: 'E2E Reprint Seller', email: `s-${Date.now()}@example.com` }),
  })
  const sellerId = String(((await sellerRes.json()) as { id: string }).id)

  const whRes = await page.request.post(`${e2eApi}/warehouses`, {
    headers: auth,
    data: JSON.stringify({ name: 'WH Reprint', code: `wh-rep-${Date.now()}` }),
  })
  const whId = String(((await whRes.json()) as { id: string }).id)
  ...
  const gtin = '000000001234'
  const cisA = `01${gtin}21${'R'.repeat(20)}0001`
  const cisB = `01${gtin}21${'R'.repeat(20)}0002`
  const csv = `cis,sku_code\n${cisA},${sku}\n${cisB},${sku}`
  const imp = await page.request.post(`${e2eApi}/operations/marking-codes/import`, {
    headers: bearer,
    multipart: {
      seller_id: sellerId,
      pools_json: JSON.stringify([{ title: 'E2E Reprint Pool', product_ids: [productId] }]),
      files: {
        name: 'codes.csv',
        mimeType: 'text/csv',
        buffer: Buffer.from(csv),
  ...
  const initialPrintWait = page.waitForResponse(
    (r) =>
      r.request().method() === 'POST' &&
      r.url().includes('/operations/marking-codes/packaging-lines/') &&
      r.url().endsWith('/print') &&
  ...
  await page.getByTestId('ff-packaging-print-marking').click()
  await expect(page.getByTestId('marking-print-dialog')).toBeVisible()
  await Promise.all([initialPrintWait, page.getByTestId('marking-print-confirm').click()])
  const initialPrintBody = (await (await initialPrintWait).json()) as { quantity: number }
  expect(initialPrintBody.quantity).toBe(2)
  ...
  await Promise.all([reprintWait, page.getByTestId('marking-print-confirm').click()])

  const reprintReq = (await reprintWait).request()
  const reprintPayload = reprintReq.postDataJSON() as {
    reprint: boolean
    code_ids?: string[]
  }
