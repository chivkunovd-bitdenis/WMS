# WMS-517 independent auth CAdES runner

`verify-cryptopro-auth-cades.mjs` is an independent cryptographic check for the True API
browser-auth profile. It does not call the WMS adapter, CryptoPro browser mock, True API,
or any certificate/key cabinet.
...
6. deletes the temporary key, certificate, CMS, and content files in `finally`.
...
the attached/default-UCS-2LE profile independently from the browser fake, but it does
not prove GOST provider behavior, CryptoPro CSP/native-host readiness, a physical USB
token, its PIN/user-presence flow, or True API sandbox acceptance. Those remain separate
BR17/B2 acceptance layers.
