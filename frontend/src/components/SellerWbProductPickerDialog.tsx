...
export type SellerWbCatalogRow = {
  id: string
  name: string
  sku_code: string
...
type Props = {
  open: boolean
  busy: boolean
  catalog: SellerWbCatalogRow[] | null
  ...
  filterRow?: (row: SellerWbCatalogRow) => boolean
  onClose: () => void
  onApply: (selections: Record<string, number>) => void | Promise<void>
}

function wbCategories(catalog: SellerWbCatalogRow[] | null): string[] {
  if (!catalog) {
    return []
  }
...
export function SellerWbProductPickerDialog({
  open,
  busy,
  catalog,
  disabledProductIds,
  ...
  const trailingColCount = (showAvailableColumn ? 1 : 0) + 1
  const totalColCount = 6 + trailingColCount
