...
## Шаг 1. Загрузить коды селлера в пул

![Честный знак: 1 — пункт меню «Честный знак», 2 — выбор селлера, 3 — кнопка «Загрузить КМ», 4 — остаток кодов по товарам](images/chz-01-puly.png)

Экран отвечает на один вопрос: сколько кодов маркировки у селлера осталось и на какие товары они годятся. Пока коды не загружены, печатать нечего, а без напечатанного кода маркируемый заказ дальше упаковки не уедет.
...
## Шаг 2. Привязать пул к товарам

Загруженные коды сами не знают, кому их выдавать: пул привязывают к товарам, и только после этого печать по товару начинает работать.
...
## Шаг 3. Напечатать этикетки

![Окно печати кодов маркировки: 1 — переключатель «Как печатать ЧЗ и ШК ВБ», 2 — размер этикетки, 3 — сколько ЧЗ на единицу, 4 — кнопка «Печать»](images/chz-02-pechat.png)

Печать — это расход: код уходит из статуса «Доступен» в «Напечатан» и обратно в пул не возвращается. Поэтому перед печатью стоит посмотреть на количество и на переключатель раздельной печати, а не жать сразу.
...
## Шаг 4. Отдать код маркетплейсу

Этот шаг делается не здесь, а на вкладке «Упаковка и маркировка» в поставке FBS — как выглядит тот экран, показано в статье **«ФБС Wildberries»**. Здесь важно, что именно там происходит с кодом.
...
## Шаг 5. Если код испорчен или WB его не принял
