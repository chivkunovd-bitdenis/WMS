...
4. В новом документе (статус **«Черновик»**) наберите состав. Кнопка **«Добавить товар»** открывает окно «Выбор товаров» с каталогом селлера: ищите по артикулу, ШК (штрихкоду), артикулу WB или названию, проставьте количество в колонке «Кол-во в заявку» и нажмите **«Добавить в заявку»**. В черновике работает и сканер: отсканируйте ШК товара, и строка добавится сама.
...
- **«Нельзя уменьшить принятое количество ниже уже размещённого».** Часть этого товара уже разложена по ячейкам, и опустить факт ниже размещённого нельзя — иначе на складе повиснет товар, которого по документам не принимали. Разбирайтесь через сортировку и остатки, а не правкой факта.
- **В документе нет кнопок «Начать приёмку» и «Завершить приёмку».** Очередь приёмок открывается всем, у кого есть доступ к разделу, а вот действия внутри документа доступны только роли администратора фулфилмента. Если кнопок нет — это права, а не поломка: позовите того, у кого они есть.
