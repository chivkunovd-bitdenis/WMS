...
type Measurement = {
  ...
  // Габариты и штуко-дни показывают, из чего вырос литраж: какая коробка и
  // сколько её лежало. Без них строка «12,4 л·дн» ничего не объясняет.
...
// в таблице читается как ошибка. Показываем столько знаков, сколько нужно
// человеку, и разделяем разряды.
...
const dimensionsLabel = (sides: number[] | null | undefined) =>
  sides && sides.length === 3 ? sides.join(' × ') : '—'
// Штуко-дни делим на дни месяца: кладовщику понятнее «в среднем 12 шт.
// лежало», чем «372 штуко-дня».
const averageStock = (quantityDays: string | null | undefined, month: string) => {
  const value = Number(quantityDays)
  if (!Number.isFinite(value) || value <= 0) return '—'
  const [year, monthNumber] = month.split('-').map(Number)
  const days = new Date(Date.UTC(year, monthNumber, 0)).getUTCDate()
  return (value / days).toFixed(1).replace('.', ',')
}
...
export function FfStoragePage({ isFulfillmentAdmin, token }: { isFulfillmentAdmin: boolean; token: string }) {
  ...
  return <Box data-testid="ff-storage-page" sx={{ minWidth: 0, width: { xs: 'calc(100vw - 48px)', md: 'calc(100vw - 308px)' }, maxWidth: '100%' }}>
    ...
    <Box sx={{ mt: 2 }}>
      ...
      {expanded && <Box sx={{ p: 2, mt: 1, minWidth: 0, maxWidth: '100%', overflow: 'hidden', borderLeft: 4, borderColor: 'primary.main', '& [data-testid="storage-sku-table"]': { maxWidth: '100%', overflowX: 'auto' }, '& [data-testid="storage-sku-table"] .MuiTable-root': { tableLayout: 'fixed', width: '100%' }, '& [data-testid="storage-sku-table"] .MuiTableCell-root': { px: 1, overflow: 'hidden', textOverflow: 'ellipsis' } }} data-testid="storage-detail">
        ...
        <DataTable columns={[
          ...
          { key: 'volume', header: 'Объём, л', width: 90, align: 'right', render: (row: Measurement) => decimal(row.volume_liters, 2) },
          { key: 'stock', header: 'В среднем лежало, шт.', width: 145, align: 'right', render: (row: Measurement) => averageStock(row.quantity_days, month) },
          { key: 'source', header: 'Источник', width: 105, render: (row: Measurement) => <TextCell value={sourceLabel(row.dimensions_source)} /> },
          { key: 'liter-days', header: 'Л-дни', width: 90, align: 'right', render: (row: Measurement) => row.status === 'missing_dimensions' ? '—' : decimal(row.liter_days, 2) },
          { key: 'rate', header: 'Ставка, ₽/л·д.', width: 135, align: 'right', render: (row: Measurement) => decimal(row.rate_snapshot, 2) },
          { key: 'amount', header: 'Сумма, ₽', width: 100, align: 'right', render: (row: Measurement) => row.status === 'missing_dimensions' ? '—' : decimal(row.amount, 2) },
    ...
    <Dialog open={Boolean(printStatement)} onClose={() => setPrintStatement(null)} maxWidth="md" fullWidth><DialogTitle>Расчёт хранения за {formatMonth(month)}</DialogTitle><DialogContent>{printStatement && <Box data-testid="storage-print-preview" sx={{ '& [data-testid="storage-print-table"] .MuiTable-root': { tableLayout: 'fixed', width: '100%' }, '& [data-testid="storage-print-table"] .MuiTableCell-root': { overflow: 'hidden', textOverflow: 'ellipsis' }, '& [data-testid="storage-print-table"] .MuiTableCell-head': { px: 0.5, fontSize: 11, whiteSpace: 'nowrap' }, '@media print': { '& .MuiTableCell-root': { px: 0.5, py: 0.5, fontSize: 10 } } }}><Typography>Селлер: {printStatement.seller_name}</Typography><Typography>Склад: {printStatement.warehouse_name}</Typography><Typography>Зафиксирован: {printStatement.fixed_at ? new Date(printStatement.fixed_at).toLocaleString('ru-RU') : '—'}</Typography><DataTable columns={[{ key: 'sku', header: 'SKU', width: 90, render: (row: Measurement) => row.sku }, { key: 'article', header: 'Артикул продавца', width: 110, render: (row: Measurement) => row.seller_article ?? '—' }, { key: 'dimensions', header: 'Габариты, мм', width: 110, align: 'right', render: (row: Measurement) => dimensionsLabel(row.dimensions_mm) }, { key: 'volume', header: 'Объём, л', width: 75, align: 'right', render: (row: Measurement) => decimal(row.volume_liters, 2) }, { key: 'source', header: 'Источник', width: 90, render: (row: Measurement) => sourceLabel(row.dimensions_source) }, { key: 'days', header: 'Литро-дни', width: 85, align: 'right', render: (row: Measurement) => decimal(row.liter_days, 2) }, { key: 'rate', header: 'Ставка, ₽/л·день', width: 125, align: 'right', render: (row: Measurement) => decimal(row.rate_snapshot, 2) }, { key: 'amount', header: 'Сумма, ₽', width: 85, align: 'right', render: (row: Measurement) => decimal(row.amount, 2) }]} rows={printStatement.measurements} getRowKey={(row) => row.product_id} empty={{ title: 'В выбранном месяце хранения не было' }} testId="storage-print-table" /><Typography sx={{ mt: 2, fontWeight: 700, textAlign: 'right' }}>Итого: {decimal(printStatement.total_amount, 2)} ₽</Typography></Box>}</DialogContent><DialogActions><SecondaryAction onClick={() => setPrintStatement(null)}>Закрыть</SecondaryAction><PrintAction what="накладную" placement="panel" onClick={() => window.print()} /></DialogActions></Dialog>
  </Box>
}
