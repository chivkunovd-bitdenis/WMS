...
export function buildInvoicePrintHtml(invoice: Invoice): string {
  const profileHtml = (profile: BillingProfileSnapshot | undefined, fallback: string) => profileRows(profile, fallback)
    ...
    .join('')
  const lineHtml = (invoice.lines ?? []).map((line) => `<tr><td>${escapeHtml(serviceLabels[line.service_code] ?? '—')}</td><td>${escapeHtml(unitLabels[line.unit] ?? '—')}</td><td>${escapeHtml(line.quantity)}</td><td>${escapeHtml(formatMoney(Number(line.rate)))}</td><td>${escapeHtml(formatMoney(Number(line.amount)))}</td></tr>`).join('')
