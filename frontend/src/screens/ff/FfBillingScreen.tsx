...
export function InvoiceDocumentDetails({ line, period }: { line: InvoiceLine; period: string }) {
  return <Stack data-testid="billing-invoice-documents" spacing={0.5}>{(line.documents ?? []).map((doc) => <Stack key={`${doc.date}-${doc.number}`} direction="row" spacing={1} sx={{ alignItems: 'baseline', flexWrap: 'wrap' }}><Typography component="span">Исходный документ: {formatMoscowDate(doc.date)} · {line.service_code === STORAGE_SERVICE_CODE ? `Расчёт хранения за ${formatPeriod(period)}` : doc.number} ·</Typography><QtyCell value={doc.quantity} /><Typography component="span">·</Typography><MoneyCell value={doc.amount / 100} /></Stack>)}</Stack>
}
