...
// WMS-542: реконсиляция ответа на скан (разделение ручной правки и
// оптимистичного прироста скана, F1–F3 из ревью Astra №1) переехала в
// отдельный чистый модуль scanReconciliation.ts и покрыта тестами там
// (scanReconciliation.test.ts) — точными последовательностями сетевых
// ответов, а не отдельными вызовами merge-функции с уже готовым touched.
