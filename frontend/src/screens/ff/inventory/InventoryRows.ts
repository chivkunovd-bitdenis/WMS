...
// WMS-542: две прошлые версии реконсиляции ответа на скан жили здесь
// (confirmedTouchedIds, applyScanResponse) и обе оказались недостаточны —
// ревью Astra №1 (docs/reviews/artifacts/wms-542/review-astra-1.md, F1–F3)
// поймало двойной счёт и потерю чужих сканов на границе ручной правки и
// оптимистичного прироста скана. Замена — отдельный чистый модуль
// scanReconciliation.ts: он явно разделяет слой ручных правок (с версией
// правки на строку) и слой сканов, вместо того чтобы пытаться различить их
// одним общим множеством «тронуто». Здесь заново не заводим.
