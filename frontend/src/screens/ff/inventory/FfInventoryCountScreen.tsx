...
export function FfInventoryCountScreen({
  ...
  function handleScan(code: string) {
    ...
    if (result.found) {
      ...
      //
      // Порядок важен: находку кладём в очередь ДО onChange ниже. Очередь
      // берёт снимок документа для отправки синхронно, в этот же момент —
      // раньше, чем onChange применит локальный прирост этой же строки.
      // Иначе несохранённая ручная правка ушла бы на сервер уже с этим
      // приростом внутри, и серверный +1 сверху задвоил бы штуку (WMS-542).
