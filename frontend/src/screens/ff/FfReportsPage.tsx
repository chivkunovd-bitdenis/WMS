...
type Props = { token: string; sellers?: { id: string; name: string }[] }
type Overview = {
  current_balance: number
  in_qty: number
  out_qty: number
  comparison: { previous_out_qty: number; change_percent: number | null; change: number }
  daily: { date: string; in_qty: number; out_qty: number }[]
  generated_at: string
...
export function FfReportsPage({ token, sellers = [] }: Props) {
  ...
  return <Stack spacing={0} data-testid="ff-reports-page">
    ...
    <FilterBar search={search} onSearchChange={setSearch} searchPlaceholder="Название, артикул продавца, SKU, ШК" testId="ff-reports-filters">
      <TextField select size="small" label="Период" value={period} onChange={event => choosePeriod(event.target.value)} data-testid="ff-reports-period"><MenuItem value="7">7 дней</MenuItem><MenuItem value="30">30 дней</MenuItem><MenuItem value="month">Текущий месяц</MenuItem><MenuItem value="year">Текущий год</MenuItem></TextField>
      ...
      <TextField size="small" label="С" type="date" value={dateFrom} onChange={event => { setPeriod('custom'); setDateFrom(event.target.value) }} data-testid="ff-reports-date-from" slotProps={{ inputLabel: { shrink: true } }} />
