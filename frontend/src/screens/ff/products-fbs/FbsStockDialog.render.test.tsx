...
function product(
  ...
  rules: Record<string, { free: number; applicable?: boolean; mode?: 'percent' | 'units'; value?: number; publish?: boolean; unitsConfigured?: boolean }>,
): StockDialogProduct {
  return {
    ...
    byBinding: Object.fromEntries(Object.entries(rules).map(([bindingId, one]) => [bindingId, toProductBindingState({
      publish: one.publish ?? true, mode: one.mode ?? 'percent', value: one.value ?? 0,
      units_configured: one.unitsConfigured ?? one.mode === 'units', marketplace: 'wb',
...
describe('WMS-469 F2/F6: сохранённый лимит и время записи', () => {
  ...
  it('WMS-483: незаданный поштучный лимит пуст, а явный ноль показан нулём', () => {
    const unset = render({ products: [product('p', 'Футболка', {
      'b-wb': { free: 30, mode: 'units', value: 0, unitsConfigured: false },
    })] })
    const zero = render({ products: [product('p', 'Футболка', {
      'b-wb': { free: 30, mode: 'units', value: 0, unitsConfigured: true },
    })] })
    expect(tag(unset, 'fbs-stock-units-b-wb')).toContain('value=""')
