...
describe('WMS-469 R9 галка «процентом» — одно значение в двух представлениях', () => {
  ...
  it('процент → число: один товар получает число из поля', () => {
    const draft: BlockDraft = { publish: true, byPercent: true, percent: 50, units: 0 }
    expect(toggleByPercent(wb, draft, false, [fifty])).toMatchObject({ byPercent: false, units: 25 })
  })

  it('процент → число при нескольких товарах: среднее на товар, не выше потолка самого малого', () => {
    const draft: BlockDraft = { publish: true, byPercent: true, percent: 100, units: 0 }
    // В поле 80 на двоих → 40 на товар, но у второго всего 30.
    expect(toggleByPercent(wb, draft, false, [fifty, thirty])).toMatchObject({ byPercent: false, units: 30 })
  })
})
...
describe('WMS-469 черновики и суммы блока', () => {
  it('черновик берётся из сохранённого правила: units → ручной режим, percent → процентный', () => {
    expect(draftFromState({ publish: true, mode: 'units', value: 30, applicable: true, onHand: 50, reserved: 0, freeStock: 50 }))
      .toEqual({ publish: true, byPercent: false, percent: 0, units: 30 })
    expect(draftFromState({ publish: false, mode: 'percent', value: 40, applicable: true, onHand: 50, reserved: 0, freeStock: 50 }))
      .toEqual({ publish: false, byPercent: true, percent: 40, units: 0 })
    expect(draftFromState(undefined)).toEqual({ publish: false, byPercent: true, percent: 0, units: 0 })
  })
...
describe('WMS-469 R7 тело сохранения', () => {
  it('отправляет только видимые блоки, с которых принимаем заказы; режим — ровно один', () => {
    ...
    expect(ruleBodyFromDrafts([wb, notServed], drafts)).toEqual({
      'b-wb': { publish: true, mode: 'units', value: 30 },
    })
    expect(ruleBodyFromDrafts([wb, ozon], drafts)).toEqual({
      'b-wb': { publish: true, mode: 'units', value: 30 },
      'b-ozon': { publish: true, mode: 'percent', value: 100 },
    })
  })
