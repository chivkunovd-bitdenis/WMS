...
export function emptyDraft(): BlockDraft {
  return { publish: false, byPercent: true, percent: 0, units: 0 }
}
...
export function draftFromState(state: ProductBindingState | undefined): BlockDraft {
  ...
  return {
    ...
    units: state.mode === 'units' ? state.value : 0,
  }
}
...
export function rowCalc(
  ...
  draft: BlockDraft,
  products: StockDialogProduct[],
): { free: number; ship: number; pct: number; fieldValue: number } {
  let free = 0
  let ship = 0
  for (const product of products) {
    ...
    else ship += Math.min(draft.units, f)
  }
...
export function toggleByPercent(
  ...
  // В число уходит то, что было в поле: при нескольких товарах — на каждый в
  // среднем, но не больше потолка самого малого из них.
  const perProduct = products.length > 1
    ? Math.round(calc.fieldValue / products.length)
    : calc.fieldValue
  return { ...draft, byPercent: false, units: clampUnits(binding, perProduct, products).units }
}
...
): Record<string, { publish: boolean; mode: 'percent' | 'units'; value: number }> {
  const body: Record<string, { publish: boolean; mode: 'percent' | 'units'; value: number }> = {}
  for (const binding of bindings) {
    ...
    body[binding.id] = draft.byPercent
      ? { publish: draft.publish, mode: 'percent', value: draft.percent }
      : { publish: draft.publish, mode: 'units', value: draft.units }
  }
  return body
}
