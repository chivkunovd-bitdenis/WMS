...
function stockErrorText(code: string | null): string | null {
  ...
  const labels: Record<string, string> = {
    ...
    ozon_stock_rejected:
      'Ozon не принял часть остатков. Проверьте склад и карточки товаров в кабинете Ozon: товар может быть в архиве, склад — неактивен, а остаток одного товара нельзя обновлять чаще раза в 30 секунд.',
    ozon_stock_unconfirmed:
      'Ozon ответил без результата по остаткам — публикация не подтверждена. Запустите синхронизацию заново.',
    ozon_stock_item_invalid:
      'В остатке нет склада Ozon или идентификатора товара. Проверьте привязку склада и связку товара с карточкой Ozon.',
  }
